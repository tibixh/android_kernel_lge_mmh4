...
//#define SUPPORT_OTHERS // Support others fingerprint.according LGE

#ifdef SUPPORT_ONE_BINARY
extern int lge_get_fingerprint_hwinfo(void);
#endif

#ifdef SUPPORT_OTHERS
#ifndef FP_ID_VALUE
#define FP_ID_VALUE (0) // FP_ID is default low
#endif

static int ff_fp_id = -1;

static int get_fp_id_from_gpio(void);
#endif // SUPPORT_OTHERS
		...
		size_t count)
{
	...
	if (*buffer == '1')
	{
		g_context->focal_id = 1;
	}
	else
	{
		g_context->focal_id = 0;
	}
...
static int __init ff_ctl_driver_init(void)
{
...
#ifdef SUPPORT_ONE_BINARY
	{
		...
		if (!is_support) {
			printk(KERN_WARNING "[ff] %s: doesn't support fingerprint, skip register\n", __func__);

			return 0;
		}

		printk(KERN_INFO "[ff] %s: support fingerprint\n", __func__);
	}
#endif // SUPPORT_ONE_BINARY

#ifdef SUPPORT_OTHERS
	{
		int fp_val = get_fp_id_from_gpio();

		if (fp_val != FP_ID_VALUE) {
			printk(KERN_WARNING "[ff] %s: fp_id mismatch, value = %d\n", __func__, fp_val);
			return -1;
		}

		printk(KERN_INFO "[ff] %s: fp_id value = %d\n", __func__, fp_val);
