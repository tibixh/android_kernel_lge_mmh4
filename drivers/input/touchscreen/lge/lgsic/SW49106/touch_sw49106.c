...
extern int primary_display_esd_report_touchintpin_keep_low(void);
			     ...
			     const struct firmware *fw)
{
	...
	//--------- Binary Check Verification Start-----------------------
		...
		if(fw->size > FLASH_SIZE) {
			TOUCH_I("%s - FW Image Size(0x%zx) is not correct\n", __func__,fw->size);
			return -EPERM;
		} else {
			TOUCH_I("%s - FW Image Size(0x%zx) FLASH_FW_SIZE= 0x%x\n",__func__,fw->size,FLASH_FW_SIZE);
		}
