...
//	u8 addr = 0;
	...
	TOUCH_I("swipe fail count : %d, index : %d\n", count, index);

//	for (i = 1; i <= count; i++) {
//
//		if (i == SWIPE_MAX_BUFFER)
//			break;
//	}
...
static void synaptics_swipe_control(struct device *dev)
{
	...
	TOUCH_I("swipe mode : %d\n", ts->swipe.mode);
	if(ts->swipe.mode == 1)
		return ;

	synaptics_set_page(dev, LPWG_PAGE);

	synaptics_write(dev, SWIPE_REPORT_ENABLE, &ts->swipe.mode, sizeof(ts->swipe.mode));
...
//		synaptics_swipe_getdata(dev, ts->tci.info[TCI_2].tap_count);
		TOUCH_I("swipe irq\n");
...
/*
static ssize_t show_swipe_enable(struct device *dev, const char *buf)
{
	struct touch_core_data *ts = to_touch_core(dev);
	int offset = 0;

	offset += snprintf(buf + offset, PAGE_SIZE - offset, "SWIPE mode = 0x%X\n", ts->swipe.mode);
