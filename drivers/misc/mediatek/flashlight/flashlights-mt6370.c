...
static const unsigned char mt6370_strobe_level[MT6370_LEVEL_FLASH] = {
#if defined(CONFIG_MACH_MT6762_MH5LM)
	...
	0x0C, 0x0C, 0x0C, 0x0C, 0x0C, 0x0C, 0x0C, 0x0C, 0x0C, 0x0C,
	0x0C, 0x0C, 0x0C, 0x0C, 0x0C, 0x0C, 0x0C, 0x14, 0x1C, 0x24,
...
static int mt6370_disable(void)
{
    ...
    torchLevel = 0;//[LGE_UPDATE] [kyunghun.oh@lge.com] [2018-11-02] enable the setting of front flash torch level
	if (ret < 0)
...
static int mt6370_set_level(int channel, int level)
{
	...
	else if (channel == MT6370_CHANNEL_CH2 && torchLevel == 0)//[LGE_UPDATE] [kyunghun.oh@lge.com] [2018-11-02] enable the setting of front flash torch level
...
#if defined(CONFIG_MACH_MT6762_MH4)
/*LGE_CHANGE_S, 2018-09-19, Implemented Flash Torch level, soojong.jin@lge.com*/
static int mt6370_flash_torch_level(int level, int channel)
{
    if(level > 14)
        level = 14;
    if (!mt6370_is_torch(level)) {
/*LGE_CHANGE_S, Implemented Flash Torch level for each channel, 2019-02-14, seungmin.hong@lge.com*/
		if (channel == MT6370_CHANNEL_CH1){
			printk("set back flash torch level(IDX) = %d\n", level);
			flashlight_set_torch_brightness(
					flashlight_dev_ch1, mt6370_torch_level_for_app[level]);
			torchLevel = level;
		}
		else if (channel == MT6370_CHANNEL_CH2) {
		printk("set front flash torch level(IDX) = %d\n", level);
			flashlight_set_torch_brightness(
					flashlight_dev_ch2, mt6370_torch_level_for_app[level]);
			torchLevel = level;
		}
		else {
...
#else
/*LGE_CHANGE_S, 2018-09-19, Implemented Flash Torch level, soojong.jin@lge.com*/
static int mt6370_flash_torch_level(int level)
{
    if(level > 14)
        level = 14;
    if (!mt6370_is_torch(level)) {
...
//[LGE_UPDATE] [bk.bae@lge.com] [2019-06-21] block flash mode in the front flash for mmh4
#if defined(CONFIG_MACH_MT6762_MH4)
				mt6370_en_ch2 = MT6370_ENABLE_TORCH;
#else
				mt6370_en_ch2 = MT6370_ENABLE_FLASH;
#endif
//[LGE_UPDATE] [bk.bae@lge.com] [2018-06-21] block flash mode in the front flash for mmh4
	} else {
		pr_err("Error channel\n");
		return -1;
	}
...
#if defined(CONFIG_MACH_MT6762_MH4)
		pr_err("[LGE] FLASH_IOC_SET_FLASH_TORCH_LEVEL(%d) Channel(%d)\n", fl_arg->arg, channel);
		fl_arg->arg = mt6370_flash_torch_level(fl_arg->arg, channel); //LGE_CHANGE_S, Implemented Flash Torch level for each channel, 2019-02-14, seungmin.hong@lge.com
#else
		pr_err("[LGE] FLASH_IOC_SET_FLASH_TORCH_LEVEL(%d)\n", channel);
		fl_arg->arg = mt6370_flash_torch_level(fl_arg->arg);
#endif
		break;
/*LGE_CHANGE_E, 2018-09-19, Implemented Flash Torch level, soojong.jin@lge.com*/
...
#if defined(CONFIG_MACH_MT6762_MH4)
/*LGE_CHANGE_S, Implemented Flash Torch level for each channel, 2019-02-14, seungmin.hong@lge.com*/
	mt6370_flash_torch_level(0, MT6370_CHANNEL_CH1); /*LGE_CHANGE, 2018-09-19, Implemented Flash Torch level, soojong.jin@lge.com*/
	mt6370_flash_torch_level(0, MT6370_CHANNEL_CH2); /*LGE_CHANGE, 2018-09-19, Implemented Flash Torch level, soojong.jin@lge.com*/
/*LGE_CHANGE_E, Implemented Flash Torch level for each channel, 2019-02-14, seungmin.hong@lge.com*/
#else
	mt6370_flash_torch_level(0); /*LGE_CHANGE, 2018-09-19, Implemented Flash Torch level, soojong.jin@lge.com*/
#endif
