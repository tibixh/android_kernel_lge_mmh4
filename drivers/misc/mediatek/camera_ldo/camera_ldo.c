...
void camera_ldo_set_en_ldo(CAMERA_LDO_SELECT ldonum,unsigned int en)
{
	...
	unsigned int value =0;
	 ...
	 if (NULL == camera_ldo_i2c_client) {
	        pr_err("[camera_ldo] camera_ldo_i2c_client is null!!\n");
	        return ;
        }
	...
	if(ret <0)
	{
		pr_err("[camera_ldo] camera_ldo_set_en_ldo read error!\n");
		return;
	}
...
void camera_ldo_set_ldo_value(CAMERA_LDO_SELECT ldonum,unsigned int value)
{
	...
	{
		pr_err("[camera_ldo] error ldonum not support!!!\n");
		return;
	}
...
if (IS_ERR(pctrl)) {
//CAMERA_LDO_PRINT("[soojong][camera_ldo]%s, pctrl %p\n",__FUNCTION__,pctrl);
pr_err("[LGE][camera_ldo]%s,dev name %s\n",__FUNCTION__,pdev->name);
pr_err("[LGE][camera_ldo]%s,dev id %d\n",__FUNCTION__,pdev->id);
}
else
{
pr_err("[LGE][camera_ldo]%s, OK\n",__FUNCTION__);
}
...
{
    if (NULL == client) {
        pr_err("[camera_ldo] i2c_client is NULL\n");
        return -1;
    }
    camera_ldo_i2c_client = client;
#if 0 //soojong, this code makes a kernel crash by BUG_ON
...
static int __init camera_ldo_init(void)
{
    ...
    if (i2c_add_driver(&camera_ldo_i2c_driver)) {
        pr_err("[camera_ldo]Failed to register camera_ldo_i2c_driver!\n");
        return -1;
    }

    if (platform_driver_register(&camera_ldo_platform_driver)) {
        pr_err("[camera_ldo]Failed to register camera_ldo_platform_driver!\n");
