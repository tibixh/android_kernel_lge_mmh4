...
#endif
{
	...
	case CAM_CALIOC_G_READ:
		...
		else if(ptempbuf->sensorID==0x0847 && pcmdInf != NULL)
		{
			int i = 0;
			u8 *temp = pu1Params;
			...
			HI846_OTP_write_cmos_sensor(0x070f, 0x05);//OTP r/w mode
			HI846_OTP_write_cmos_sensor(0x0a00, 0x01);//standby off

			HI846_OTP_write_cmos_sensor(0x070a, 0x2); //start address H
			HI846_OTP_write_cmos_sensor(0x070b, 0x1); //start address L
			HI846_OTP_write_cmos_sensor(0x0702, 0x01); //read enable
		   for (i = 0; i< ptempbuf->u4Length; i++) {
			   if (pcmdInf->readCMDFunc != NULL)
				   i4RetValue =
					   pcmdInf->readCMDFunc(pcmdInf->client,0x0708,temp,1);
			   temp++;
		   }
		   OTP_write_cmos_sensor(0x0a00, 0x00); //complete
		   udelay(100);
		   OTP_write_cmos_sensor(0x003e, 0x00); //complete
		   OTP_write_cmos_sensor(0x0a00, 0x01); //complete
