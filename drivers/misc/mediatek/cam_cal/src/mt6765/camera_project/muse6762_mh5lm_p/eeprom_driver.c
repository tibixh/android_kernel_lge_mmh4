			      ...
			      struct stCAM_CAL_CMD_INFO_STRUCT *cmdInfo)
{
	...
	printk("%s end! deviceID=%d index=%u client=%p\n",
...
#endif
{
	...
	int index = 0;
	m290_otg_data_t* pm290_otp_data = NULL;
	...
	case CAM_CALIOC_G_READ:
		...
		if (ptempbuf->sensorID == 0x8044 || ptempbuf->sensorID ==0x846)
			pm290_otp_data = &(m290_otp_data[0]);
		else if (ptempbuf->sensorID == 0x556 || ptempbuf->sensorID == 0x5035) {
			pm290_otp_data = &(m290_otp_data[1]);
		}
		i4RetValue = ptempbuf->u4Length;
		switch (ptempbuf->u4Offset) {
		case M290_AWB_5100K_FLAG:
			*pu1Params = pm290_otp_data->awb_5100K_flag;
			break;
		case M290_AWB_3000K_FLAG:
			*pu1Params = pm290_otp_data->awb_3000k_flag;
			break;
		case M290_AF_FLAG:
			*pu1Params = pm290_otp_data->af_flag;
			break;
		case M290_LSC_5100K_FLAG:
			*pu1Params = pm290_otp_data->lsc_5100_flag;
			break;
		case M290_LSC_4000K_FLAG:
			*pu1Params = pm290_otp_data->lsc_3000k_flag;
			break;

		case M290_AWB_5100K_DATA:
			for (index =0; index < ptempbuf->u4Length; index++) {
				*(pu1Params+index) = pm290_otp_data->awb_5100k.data[index];
			}
			break;
		case M290_AWB_3000K_DATA:
			for (index =0; index < ptempbuf->u4Length; index++) {
				*(pu1Params+index) = pm290_otp_data->awb_3000k.data[index];
			}
			break;
		case M290_AF_DATA:
			for (index =0; index < ptempbuf->u4Length; index++)
				*(pu1Params+index) = pm290_otp_data->af.data[index];
			break;
		case M290_LSC_5100K_DATA:
			for (index =0; index < ptempbuf->u4Length; index++)
				*(pu1Params+index) = pm290_otp_data->lsc_5100k.data[index];
			break;
		case M290_LSC_4000K_DATA:
			for (index =0; index < ptempbuf->u4Length; index++)
				*(pu1Params+index) = pm290_otp_data->lsc_3000k.data[index];
			break;
		default:
			printk("No menu M290:big error ptempbuf->u4Offset(%d)\n", ptempbuf->u4Offset);
			i4RetValue = -EPERM;
			break;
		}
