...
#if defined(CONFIG_MACH_MT6762_MH4X) || defined(CONFIG_MACH_MT6762_MH4P)
#define MAX_VALUE_SETTINGS 215 /* value leds_brightness_set*/
#else
...
#ifdef CONFIG_LGE_USE_BRIGHTNESS_TABLE
	if(blmap_arr != NULL)
		bright = (uint32_t)blmap_arr[MAX_VALUE_SETTINGS];
#endif
...
static inline int mt_parse_dt(struct device *dev)
{
	...
	if (of_property_read_u32(np, "mt,max_bled_brightness", &tmp) < 0)
		pdata->max_bled_brightness = 1023;
	else
