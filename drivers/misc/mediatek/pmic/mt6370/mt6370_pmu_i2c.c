...
#ifdef CONFIG_LGE_PM_DEBUG_FS
const static int regs_to_dump[] = {
	MT6370_PMU_REG_DEVINFO,
...
{
	struct mt6370_pmu_chip *chip = data;
	int ret;

	ret = mt6370_pmu_reg_read(chip, chip->debug_addr);
	if (ret < 0)
		return ret;
...
static int debugfs_set_data(void *data, u64 val)
{
	...
	ret = mt6370_pmu_reg_write(chip, chip->debug_addr, temp);
	if (ret < 0)
		return ret;
...
{
	struct mt6370_pmu_chip *chip = m->private;
	int ret;
	int i;

	for (i = 0; i < ARRAY_SIZE(regs_to_dump); i++) {
		ret = mt6370_pmu_reg_read(chip, regs_to_dump[i]);
		if (ret < 0) {
...
{
	struct mt6370_pmu_chip *chip = inode->i_private;
...
static int create_debugfs_entries(struct mt6370_pmu_chip *chip)
{
	struct dentry *ent;

	if (!chip->debugfs) {
		chip->debugfs = debugfs_create_dir("mt6370_pmu", NULL);
		if (!chip->debugfs) {
