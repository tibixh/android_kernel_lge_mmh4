...
static int mt6370_safety_check(struct charger_device *chg_dev)
{
	...
	if (adc_ibat <= 300000)
		counter++;
#ifdef CONFIG_LGE_PM
	else if (adc_ibat <= chg_data->ieoc - 150000)
		counter++;
#endif
	else
		counter = 0;
...
#ifdef CONFIG_LGE_PM
static int mt6370_input_suspend(struct charger_device *chg_dev, bool en)
{
	struct mt6370_pmu_charger_data *chg_data =
		dev_get_drvdata(&chg_dev->dev);
...
static int mt6370_is_input_suspended(struct charger_device *chg_dev, bool *en)
{
	int ret = 0;
	struct mt6370_pmu_charger_data *chg_data =
		dev_get_drvdata(&chg_dev->dev);
...
#ifdef CONFIG_LGE_PM
	/* Input Control */
	.input_suspend = mt6370_input_suspend,
	.is_input_suspended = mt6370_is_input_suspended,
