...
#define  HI556_I2C_ADDR 0x40
static void OTP_write_cmos_sensor(kal_uint32 addr, kal_uint32 para)
{
...
static void hi556_otp_setting(UINT32 addr)
{
	LOG_INF("%s: Enter!!", __func__);

	OTP_write_cmos_sensor(0x0A02, 0x01); //Fast sleep on
	OTP_write_cmos_sensor(0x0A00, 0x00);//stand by on
	mdelay(100);
	OTP_write_cmos_sensor(0x0F02, 0x00);//pll disable
	OTP_write_cmos_sensor(0x011a, 0x01);//CP TRIM_H
	OTP_write_cmos_sensor(0x011b, 0x09);//IPGM TRIM_H
	OTP_write_cmos_sensor(0x0d04, 0x01);//Fsync(OTP busy)Output Enable
	OTP_write_cmos_sensor(0x0d00, 0x07);//Fsync(OTP busy)Output Drivability
	OTP_write_cmos_sensor(0x003e, 0x10);//OTP r/w mode
	OTP_write_cmos_sensor(0x0a00, 0x01);//standby off

	OTP_write_cmos_sensor(0x010a, (addr >> 8) & 0xFF); //start address H
	OTP_write_cmos_sensor(0x010b, (addr & 0xFF)); //start address L
	OTP_write_cmos_sensor(0x0102, 0x01); //read enable

	LOG_INF("%s: Exit!!", __func__);
}

static void hi556_read_module_info(void)
{
	kal_uint8 flag = 0;
	u8 *temp = (u8*)&pOtp_data;

	LOG_INF("%s: Enter!!", __func__);

	for (flag = 0; flag< 10; flag++) {
		*temp = read_cmos_sensor(0x0108);
		temp++;
		mdelay(4);
	}
	OTP_write_cmos_sensor(0x0a00, 0x00);
	mdelay(100);
	OTP_write_cmos_sensor(0x003e, 0x00);
	OTP_write_cmos_sensor(0x0a00, 0x01);

	hw_info_sub_otp.otp_valid = pOtp_data.vaild_flag;
	hw_info_sub_otp.vendor_id = pOtp_data.supplier_code;
	...
	hw_info_sub_otp.vcm_vendorid = 0xff;
	hw_info_sub_otp.vcm_moduleid = 0xff;
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
			...
			do {
				*sensor_id =return_sensor_id();
				...
				#ifdef CONFIG_HQ_HARDWARE_INFO
				sensor_init();
				hi556_otp_setting(0x0401);
				hi556_read_module_info();
				#endif	
				return ERROR_NONE;
				}	
			  ...
			  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	imgsensor.autoflicker_en = KAL_FALSE;
	spin_unlock(&imgsensor_drv_lock);
	capture_setting(imgsensor.current_fps); //normal_video_setting bk.bae@lge.com fullsize recording
	return ERROR_NONE;
}	/*	normal_video   */
...
UINT32 HI556_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
	...
	printk("HI556_MIPI_RAW_SensorInit in\n");
	if (pfFunc!=NULL)
		*pfFunc=&sensor_func;
	return ERROR_NONE;
