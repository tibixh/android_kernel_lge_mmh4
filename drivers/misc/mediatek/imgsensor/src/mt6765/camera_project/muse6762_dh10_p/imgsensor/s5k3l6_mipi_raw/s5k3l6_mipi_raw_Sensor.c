...
extern unsigned int sensor_idx_flag;
...
// 20190423 add stream shy control Begin
...
// 20190423 add stream shy control end
...
static void capture_setting(kal_uint16 currefps)
{
    ...
    if (currefps == 300) {
       ...
       // write_cmos_sensor_byte(0x0100,0x00);
        ...
        write_cmos_sensor(0x3300,0x0000);
        write_cmos_sensor(0x3400,0x0000);
        write_cmos_sensor(0x3402,0x4E43);//0x4E42 Dynamic BPC ON
...
#define QRCODE_CHECKSUM_L 0X03D0
#define QRCODE_CHECKSUM_M 0X03D1
...
static void get_eeprom_data(EEPROM_DATA *data)
{
	...
	otp_data = (u8 *)data;
	for (i=0;i<=0xE;i++)
		LOG_INF_NEW(" [jian]otpdata[0x%x]=0x%x    ", i, *(otp_data + i));
#endif
	read_eeprom_barcode();
	return ;
}
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
    ...
    printk("shy %s,s5k3l6_sub sensor_idx_flag=%d",__func__,sensor_idx_flag);
    if(sensor_idx_flag !=1){
printk("shy %s sensor_idx_flag=%d,no s5k3l6_sub",__func__,sensor_idx_flag);
	...
	#else
			printk("shy s5k3l6 i2c write id: 0x%x, ReadOut sensor id: 0x%x, imgsensor_info.sensor_id:0x%x.\n", imgsensor.i2c_write_id,*sensor_id,imgsensor_info.sensor_id);
               return ERROR_NONE;
      #endif
            }
			printk("shy s5k3l6 Read sensor_id, i2c write id: 0x%x, ReadOut sensor id: 0x%x, imgsensor_info.sensor_id:0x%x.\n", imgsensor.i2c_write_id,*sensor_id,imgsensor_info.sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        retry = 1;
...
{
	LOG_INF("E\n");
    printk("[s5k3l6] Start preview mode\n");
	spin_lock(&imgsensor_drv_lock);
...
{
	LOG_INF("E\n");
    printk("[s5k3l6] Start Capture mode\n");
	spin_lock(&imgsensor_drv_lock);
...
{
	LOG_INF("E\n");
    printk("[s5k3l6] Start NormalVideo mode\n");
	spin_lock(&imgsensor_drv_lock);
...
{
    LOG_INF("E\n");
    printk("[s5k3l6] Start Custom1 mode\n");
    spin_lock(&imgsensor_drv_lock);
...
{
    LOG_INF("E\n");
    printk("[s5k3l6] Start Custom2 mode\n");
    spin_lock(&imgsensor_drv_lock);
                             ...
                             UINT8 *feature_para,UINT32 *feature_para_len)
{
    ...
    switch (feature_id) {
        ...
        case SENSOR_FEATURE_SET_IHDR_SHUTTER_GAIN:
            ...
            break;
// 20190422 add stream shy control Begin
	case SENSOR_FEATURE_SET_STREAMING_SUSPEND:
		LOG_INF("shy 3L6 SUSPENDSENSOR_FEATURE_SET_STREAMING_SUSPEND\n");
		streaming_control(KAL_FALSE);
		break;
	case SENSOR_FEATURE_SET_STREAMING_RESUME:
		LOG_INF("shy 3L6 RESUME SENSOR_FEATURE_SET_STREAMING_RESUME, shutter:%llu\n", *feature_data);
		if (*feature_data != 0)
			set_shutter(*feature_data);
		streaming_control(KAL_TRUE);
		break;
// 20190422 add stream shy control end
		#if 0
