...
static kal_uint16 read_sensor_eeprom(kal_uint32 addr)
{
	...
	iReadRegI2C(pu_send_cmd, 2, (u8 *)&get_byte, 1, HI556_QTECH_EEPROM);
...
#define QRCODE_CHECKSUM_L 0X0740
#define QRCODE_CHECKSUM_M 0X0741
...
{
	LOG_INF("E");
    printk("[HI556] Start Preview Mode\n");
	spin_lock(&imgsensor_drv_lock);
						  ...
						  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	LOG_INF("E");
	spin_lock(&imgsensor_drv_lock);
	printk("[HI556] Start Capture Mode\n");
...
{
	LOG_INF("E");
	printk("[HI556] Start NormalVideo Mode\n");
	spin_lock(&imgsensor_drv_lock);
