...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	if (*sensor_id != imgsensor_info.sensor_id) {
		printk("ov13855 get_imgsensor_id: 0x%x fail\n", *sensor_id);
...
{
	spin_lock(&imgsensor_drv_lock);
    printk("[OV13855] Start preview mode\n");
...
{
	spin_lock(&imgsensor_drv_lock);
    printk("[OV13855] Start capture mode\n");
...
{
	spin_lock(&imgsensor_drv_lock);
    printk("[OV13855] Start NormalVideo mode\n");
