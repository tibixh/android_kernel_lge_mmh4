...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
    kal_uint8 i = 0;
    kal_uint8 retry = 2;
printk("shy %s,s5k3l6_main sensor_idx_flag=%d",__func__,sensor_idx_flag);
    if(sensor_idx_flag !=0){
        printk("shy %s sensor_idx_flag=%d,no s5k3l6_main",__func__,sensor_idx_flag);
	...
	#else
			printk("shy s5k3l6_main read ok i2c write id: 0x%x, ReadOut sensor id: 0x%x, imgsensor_info.sensor_id:0x%x.\n", imgsensor.i2c_write_id,*sensor_id,imgsensor_info.sensor_id);
                return ERROR_NONE;
      #endif
            }
			printk("shy s5k3l6_main Read sensor id fail, i2c write id: 0x%x, ReadOut sensor id: 0x%x, imgsensor_info.sensor_id:0x%x.\n", imgsensor.i2c_write_id,*sensor_id,imgsensor_info.sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        retry = 1;
...
{
	LOG_INF("E\n");
    printk("[s5k3l6txd] start preview mode\n");
	spin_lock(&imgsensor_drv_lock);
...
{
	LOG_INF("E\n");
    printk("[s5k3l6txd] start capture mode\n");
	spin_lock(&imgsensor_drv_lock);
...
{
	LOG_INF("E\n");
    printk("[s5k3l6txd] start normal video mode\n");
	spin_lock(&imgsensor_drv_lock);
...
{
    LOG_INF("E\n");
    printk("[s5k3l6txd] start Custom1 mode\n");
    spin_lock(&imgsensor_drv_lock);
...
{
    LOG_INF("E\n");
    printk("[s5k3l6txd] start Custom2 mode\n");
    spin_lock(&imgsensor_drv_lock);
...
{
    LOG_INF("S5K3L6TXD_MIPI_RAW_SensorInit in\n");
