...
//soojong	.mirror = IMAGE_NORMAL,				//mirrorflip information
...
{
LOG_DEBUG("[HI556W] init setting start\n");
#if MULTI_WRITE
...
{
LOG_DEBUG("[HI556W] preview setting start\n");
#if MULTI_WRITE
	hi556_table_write_cmos_sensor(
...
{
LOG_DEBUG("[HI556W] capture setting start currentfps : %d\n", currefps);
#if MULTI_WRITE
	if (currefps == 300) {
	hi556_table_write_cmos_sensor(
			...
			do {
				*sensor_id =return_sensor_id();
				if (*sensor_id == imgsensor_info.sensor_id) {				
                    strcpy(wide_sensor_name, "hi556_qtech");/*LGE_CHANGE, 2019-07-04, add the camera identifying logic , kyunghun.oh@lge.com*/
...
{
    LOG_DEBUG("[HI556W] preview start\n");
	spin_lock(&imgsensor_drv_lock);
...
static kal_uint32 capture(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
						  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
    LOG_DEBUG("[HI556W] capture start\n");
	spin_lock(&imgsensor_drv_lock);
...
static kal_uint32 normal_video(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
					  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
    LOG_DEBUG("[HI556W] normal_video start\n");
