...
#include "s5k3l6txd_mipi_raw_Sensor.h"
...
#define LOG_1 LOG_INF("S5K3L6TXD,MIPI 4LANE\n")
...
static imgsensor_info_struct imgsensor_info = {
	.sensor_id = S5K3L6TXD_SENSOR_ID,		//Sensor ID Value: 0x30C8//record sensor id defined in Kd_imgsensor.h
  ...
  .custom1 = {
		.pclk = 480000000,				//record different mode's pclk
		.linelength  = 6176, //6075,//5808,				//record different mode's linelength
		.framelength = 3238, //3292,			//record different mode's framelength
		.startx = 0,					//record different mode's startx of grabwindow
		.starty = 0,					//record different mode's starty of grabwindow
		.grabwindow_width  = 4208,		//record different mode's width of grabwindow
		.grabwindow_height = 3120,		//record different mode's height of grabwindow
...
{
	LOG_DEBUG("[S5K3L6TXD] init_setting Start\n");
...
{
	LOG_DEBUG("[S5K3L6TXD] preview_setting Start\n");
...
{
	LOG_DEBUG("[S5K3L6TXD] capture_setting Start current fps : %d\n", currefps);
    if (currefps == 300) {
    	...
    	LOG_DEBUG("else if (currefps == 240)\n");
    	...
    	LOG_DEBUG("else if (currefps == 150)\n");
...
{
	LOG_DEBUG("[S5K3L6TXD] normal_video_setting Start current fps : %d\n", currefps);
       write_cmos_sensor(0x0100,0x0000);
...
static void custom1_setting(kal_uint16 currefps)
{
	LOG_INF("E! currefps:%d\n", currefps);
		...
		write_cmos_sensor(0x3C1E, 0x0000); 
		write_cmos_sensor(0x030C, 0x0004); 
		write_cmos_sensor(0x030E, 0x00B3); 
		write_cmos_sensor(0x3C16, 0x0001); 
		write_cmos_sensor(0x0300, 0x0006); 
		write_cmos_sensor(0x0342, 0x1820); 
		write_cmos_sensor(0x0340, 0x0CA6); 
		write_cmos_sensor(0x38C4, 0x0008); 
		write_cmos_sensor(0x38D8, 0x0025); 
		...
		write_cmos_sensor(0x38B0, 0x000D); 
		write_cmos_sensor(0x3932, 0x2000); 
		write_cmos_sensor(0x3938, 0x000C); 
		write_cmos_sensor(0x0820, 0x0432); 
		write_cmos_sensor(0x380C, 0x0090); 
		write_cmos_sensor(0x3064, 0xFFCF); 
		write_cmos_sensor(0x309C, 0x0640); 
		write_cmos_sensor(0x3090, 0x8800); 
...
#endif
	//read_eeprom_barcode(); //blocked by soojong.jin
	return ;
}
#endif
        ...
        do {
            *sensor_id = return_sensor_id() + 1;
            if (*sensor_id == imgsensor_info.sensor_id) {
                strcpy(rear_sensor_name, "s5k3l6_tsp");/*LGE_CHANGE, 2019-07-04, add the camera identifying logic , kyunghun.oh@lge.com*/
	#ifdef CONFIG_HQ_HARDWARE_INFO
...
static kal_uint32 open(void)
{
	...
	LOG_1;
	LOG_DEBUG("[S5K3L6TXD] open sensor\n");
...
{
	LOG_DEBUG("[S5K3L6TXD] close sensor\n");
...
{
	LOG_DEBUG("[S5K3L6TXD] preview Start\n");
...
static kal_uint32 capture(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
						  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	LOG_DEBUG("[S5K3L6TXD] capture Start\n");
	spin_lock(&imgsensor_drv_lock);
...
static kal_uint32 normal_video(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
					  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	LOG_DEBUG("[S5K3L6TXD] normal_video Start\n");
                             ...
                             UINT8 *feature_para,UINT32 *feature_para_len)
{
    ...
    switch (feature_id) {
		...
		case SENSOR_FEATURE_GET_PDAF_INFO:
			LOG_DEBUG("SENSOR_FEATURE_GET_PDAF_INFO scenarioId:%llu\n", *feature_data);
...
{
    LOG_DEBUG("S5K3L6TXD_MIPI_RAW_SensorInit in\n");
	/* To Do : Check Sensor status here */
	if (pfFunc!=NULL)
		*pfFunc=&sensor_func;
	return ERROR_NONE;
}	/*	S5K3L6TXD_MIPI_RAW_SensorInit	*/
