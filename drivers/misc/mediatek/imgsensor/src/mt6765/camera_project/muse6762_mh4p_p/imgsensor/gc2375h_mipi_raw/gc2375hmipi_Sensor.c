...
static void gc2375h_gcore_read_otp_info(void)
{
	...
	LOG_DEBUG("GC2375H_OTP: flag = 0x%x\n", flag);
	...
	if (0x01 == gc2375h_otp_info.wb_flag) {
		LOG_DEBUG("GC2375H_OTP_WB: Valid");
		...
		LOG_DEBUG("GC2375H_OTP_WB: Invalid!");
	}

	/* OB */
	if (0x01 == gc2375h_otp_info.ob_flag) {
		LOG_DEBUG("GC2375H_OTP_OB:Valid");
		...
		LOG_DEBUG("GC2375H_OTP_OB:Invalid!");
	}
}
...
static void gc2375h_gcore_enable_otp(kal_uint8 state)
{
	...
	if (state) {
		...
		LOG_DEBUG("GC2375H_OTP: Enable OTP!\n");
	} else {
		otp_en = otp_en & 0x7f;
		...
		LOG_DEBUG("GC2375H_OTP: Disable OTP!\n");
	}
}
