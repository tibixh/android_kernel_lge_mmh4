...
#include "sp2509v_dualmipi_Sensor.h"

#include <linux/hardware_info.h>	//For Hardwareinfo

#define EEPROM_FM34C32_ID 0xA2

//#define SP2509V_EEPROM_CALI

/****************************Modify Following Strings for Debug****************************/
#define PFX "SP2509V_DUAL_camera_sensor"
#define LOG_1 LOG_INF("SP2509V_DUAL,MIPI 1LANE\n")
...
static imgsensor_info_struct imgsensor_info = {
	.sensor_id = SP2509V_DUAL_SENSOR_ID,

	.checksum_value = 0xafe2bff,//0xed66244c,//0x3ca80b08,        //checksum value for Camera Auto Test

	.pre = {
		.pclk = 36000000,            //record different mode's pclk
		.linelength = 948,            //record different mode's linelength
...
static kal_uint32 return_sensor_id(void)
{
	...
	temp = ((read_cmos_sensor(0x02) << 8) | read_cmos_sensor(0x03)) + 0x10;  //for dual, added 0x10 to sensor id
...
LOG_INF("sensor_init_setting\n");
...

	{
		...
		write_cmos_sensor(0x06,0x2a);//0a
...
/*
#ifdef SP2509V_EEPROM_CALI
static struct sp2509v_otp_data_str pOTP_Data = {0};

static kal_uint32 read_eeprom_info_data(
		struct sp2509v_otp_data_str *pData, kal_uint16 addr)
{
...
static kal_uint32 sp2509v_otp_apply(struct sp2509v_otp_data_str *pData)
{
	// Apply AWB 
	...
	R_gain = (RGr_ratio_Typical * 0x80) / RGr_ratio;
	B_gain = (BGr_ratio_Typical * 0x80) / BGr_ratio;
	Gb_gain = (GbGr_ratio_Typical * 0x80) / GbGr_ratio;
	Gr_gain = 0x80;
	Base_gain = R_gain;
	...
	R_gain = 0x80 * R_gain / Base_gain;
	B_gain = 0x80 * B_gain / Base_gain;
	Gb_gain = 0x80 * Gb_gain / Base_gain;
	Gr_gain = 0x80 * Gr_gain / Base_gain;

	write_cmos_sensor(0xFD, 0x02);//Page 2
	LOG_INF("R_gain = 0x%x, B_gain = 0x%x, Gb_gain = 0x%x, Gr_gain = 0x%x", R_gain, B_gain, Gb_gain, Gr_gain);
	//LOG_DBG("Before Apply: [0x10] = 0x%x, [0x11] = 0x%x, [0x13] = 0x%x, [0x14] = 0x%x",
	//		read_cmos_sensor(0x10), read_cmos_sensor(0x11), read_cmos_sensor(0x13), read_cmos_sensor(0x14));
	if(Gr_gain > 0x80)
		write_cmos_sensor(0x13, Gr_gain);
	if(R_gain > 0x80)
		write_cmos_sensor(0x10, R_gain);
	if(B_gain > 0x80)
		write_cmos_sensor(0x11, B_gain);
	if(Gb_gain > 0x80)
		write_cmos_sensor(0x14, Gb_gain);
	//LOG_DBG("After Apply: [0x10] = 0x%x, [0x11] = 0x%x, [0x13] = 0x%x, [0x14] = 0x%x",
	//		read_cmos_sensor(0x10), read_cmos_sensor(0x11), read_cmos_sensor(0x13), read_cmos_sensor(0x14));

	return 1;
}

static void sp2509v_get_eeprom_data(void)
{
		...
		do {
			*sensor_id = return_sensor_id();
			...
			if (*sensor_id == imgsensor_info.sensor_id) {
                strcpy(aux_sensor_name, "sp2509v_qtech");/*LGE_CHANGE, 2019-07-04, add the camera identifying logic , kyunghun.oh@lge.com*/
				//zaki add ++++++++++++++
				//vendorId=read_eeprom(0x0001);
				LOG_DBG("Read imgsensor id ok! i2c write id: 0x%x, sensor id: 0x%x vendorID=0x%x\n",
						imgsensor.i2c_write_id, *sensor_id,vendorId);

/*				if (vendorId==0x55){
#ifdef SP2509V_EEPROM_CALI
					if ((pOTP_Data.info_flag != 0x01) || (pOTP_Data.awb_flag != 0x01))
						sp2509v_get_eeprom_data();
#endif*/
					return ERROR_NONE;
			...
			LOG_DBG("gpw Read sensor id fail, write id:0x%x id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
			retry--;
		} while(retry > 0);
		i++;
		retry = 1;
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			sensor_id = return_sensor_id();
			if (sensor_id == imgsensor_info.sensor_id) {
				LOG_INF("Read sensor id OK gpw i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
				break;
			}
...
{
LOG_DBG("SP2509V_DUAL_MIPI_RAW_SensorInit\n");
