...
#define LOG_DEBUG(format,  args...)	pr_err(PFX "[LGE][%s] " format,  __FUNCTION__,  ##args)
...
static imgsensor_info_struct imgsensor_info = {
	...
	.checksum_value = 0xb1f1b3cc, /* checksum value for Camera Auto Test */
		...
		.custom2_delay_frame = 2,	/* enter custom2 video delay frame num */
...
{
	LOG_DEBUG("[S5K3P9] Init setting start\n");
	/*Global setting */
...
{
	LOG_DEBUG("[S5K3P9] Capture setting start\n");
...
{
	LOG_DEBUG("[S5K3P9] preview setting start\n");
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = ((read_cmos_sensor_16_8(0x0000) << 8) | read_cmos_sensor_16_8(0x0001));
			LOG_DEBUG("read out sensor id 0x%x \n", *sensor_id);
			if (*sensor_id == imgsensor_info.sensor_id) {
                strcpy(rear_sensor_name, "s5k3p9_tsp");/*LGE_CHANGE, 2019-07-04, add the camera identifying logic , kyunghun.oh@lge.com*/
				#ifdef CONFIG_HQ_HARDWARE_INFO
...
{
	LOG_DEBUG("[S5K3P9] preview start\n");
...
static kal_uint32 capture(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
						  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	LOG_DEBUG("[S5K3P9] capture start\n");
	spin_lock(&imgsensor_drv_lock);
	...
	imgsensor.autoflicker_en = KAL_FALSE;
	spin_unlock(&imgsensor_drv_lock);
	pr_err("[Murphy ]%s:imgsensor.current_fps = %d\n",__func__, imgsensor.current_fps);
...
static kal_uint32 normal_video(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
					  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	LOG_DEBUG("[S5K3P9] normal_video start\n");
...
static kal_uint32 hs_video(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
					  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	LOG_DEBUG("Don't use, no setting E\n");
