...
{
	LOG_DEBUG("[S5K3L6] init_setting Start\n");
...
{
	LOG_DEBUG("[S5K3L6] preview_setting Start\n");
...
{
	LOG_DEBUG("[S5K3L6] capture_setting Start current fps : %d\n", currefps);
    if (currefps == 300) {
...
{
	LOG_DEBUG("[S5K3L6] normal_video_setting Start current fps : %d\n", currefps);
       write_cmos_sensor(0x0100,0x0000);
        ...
        do {
            *sensor_id = return_sensor_id();
            if (*sensor_id == imgsensor_info.sensor_id) {
                 strcpy(front_sensor_name, "s5k3l6_qtech");/*LGE_CHANGE, 2019-07-04, add the camera identifying logic , kyunghun.oh@lge.com*/
	#ifdef CONFIG_HQ_HARDWARE_INFO
...
{
	LOG_DEBUG("[S5K3L6] preview Start\n");
...
static kal_uint32 capture(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
						  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	LOG_DEBUG("[S5K3L6] capture Start\n");
	spin_lock(&imgsensor_drv_lock);
...
static kal_uint32 normal_video(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
					  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	LOG_DEBUG("[S5K3L6] normal_video Start\n");
...
{
    LOG_DEBUG("S5K3L6_MIPI_RAW_SensorInit in\n");
