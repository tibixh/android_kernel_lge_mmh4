...
{
	pr_info("[OV16885] PREVIEW_SETTING_E\n");
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = return_sensor_id();
			/* lx_revised */
			pr_info("[ov16885]Read sensor id fail, write id:0x%x ,sensor Id:0x%x\n",
				imgsensor.i2c_write_id, *sensor_id);
			if (*sensor_id == imgsensor_info.sensor_id) {

				pr_info("i2c write id: 0x%x, sensor id: 0x%x\n",
					imgsensor.i2c_write_id, *sensor_id);
                strcpy(rear_sensor_name, "OV16885_Qtech");// [LGE_UPDATE] [kyunghun.oh@lge.com] [2018-12-05] Creation of node for camera module identification.
				return ERROR_NONE;
			}
...
{
	LOG_INF("E\n");
    pr_info("[OV16885] preview setting start\n");
	spin_lock(&imgsensor_drv_lock);
...
/* imgsensor.autoflicker_en = KAL_FALSE; */
    ...
    pr_info("[OV16885] preview setting end\n");
	return ERROR_NONE;
}				/*      preview   */
...
{
	LOG_INF("E\n");
    pr_info("[OV16885] capture setting start\n");
	spin_lock(&imgsensor_drv_lock);
	...
	set_mirror_flip(imgsensor.mirror); ////GIONEE:malp modify
    pr_info("[OV16885] capture setting end\n");
...
static kal_uint32 normal_video(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
			       MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
    pr_info("[OV16885] normal video setting start\n");
	LOG_INF("E\n");
	...
	set_mirror_flip(imgsensor.mirror); ////GIONEE:malp modify
    pr_info("[OV16885] normal video setting end\n");
...
//Reallocation is required in the HAL.
	...
	{
		*((MUINT32 *)(feature_data)) = (0x3f)<<16 | (0x3f);     //16:9 = 63, 4:3 = 63 // (63)<<16 | (63)
