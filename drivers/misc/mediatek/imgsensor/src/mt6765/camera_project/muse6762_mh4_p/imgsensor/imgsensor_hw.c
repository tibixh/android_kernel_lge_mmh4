...
//[LGE_UPDATE_S] [kyunghun.oh@lge.com] [2018-09-28] revision check for mh4 cv1 board
        //if(lge_get_board_revno() < HW_REV_A) {
    	//	pcust_pwr_cfg = imgsensor_custom_config_mh4_rev0;
        //}
        //else {
    		pcust_pwr_cfg = imgsensor_custom_config;
        //}
//[LGE_UPDATE_E] [kyunghun.oh@lge.com] [2018-09-28] revision check for mh4 cv1 board
	...
	char *pcurr_idx)
{
	...
	struct IMGSENSOR_HW_POWER_INFO   *ppwr_info , *tppwr_info;
	...
	if (pwr_status == IMGSENSOR_HW_POWER_STATUS_OFF) {
		while (pin_cnt) {
			ppwr_info--;
			pin_cnt--;
			tppwr_info = ppwr_info;

			if(!strcmp(ppwr_seq->idx,"gc8034_mipi_raw")){
				if(ppwr_info->pin == IMGSENSOR_HW_PIN_RST){
					tppwr_info--;
				}
				...
				if(tppwr_info->pin != IMGSENSOR_HW_PIN_UNDEF) {
				pdev =phw->pdev[psensor_pwr->id[tppwr_info->pin]];
				mdelay(tppwr_info->pin_on_delay);
