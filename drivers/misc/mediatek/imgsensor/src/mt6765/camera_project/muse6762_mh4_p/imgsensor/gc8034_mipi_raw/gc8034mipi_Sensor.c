...
#ifdef GC8034_HLT_EEPROM
static kal_uint16 read_eeprom(kal_uint32 addr)
{
	kal_uint16 get_byte=0;

	char pu_send_cmd[2] = { (char)(addr >> 8), (char)(addr & 0xFF) };
	iReadRegI2C(pu_send_cmd, 2, (u8*)&get_byte, 1, EEPROM_HLT_GC8034_ID);
...
static void sensor_init(void)
{
	...
	write_cmos_sensor(0x02, 0x07);//03   add drive capability 20190228
...
static void fullsize_setting(void)
{
	...
	write_cmos_sensor(0xfe, 0x03);
	write_cmos_sensor(0x01, 0x07);
	write_cmos_sensor(0x02, 0x07);//04  add drive capability 20190228
	write_cmos_sensor(0x04, 0x80);
	write_cmos_sensor(0x11, 0x2b);
		...
		do {
			...
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
                ...
                strcpy(front_sensor_name, "GC8034_HLT");
...
static kal_uint32 open(void)
{
	...
	spin_unlock(&imgsensor_drv_lock);
    pr_info("[GC8034F] open sensor end\n");
...
{
    pr_info("[GC8034F] preview setting start\n");
	LOG_INF("E\n");
	spin_lock(&imgsensor_drv_lock);
	...
	spin_unlock(&imgsensor_drv_lock);
	preview_setting();
    pr_info("[GC8034F] preview setting end\n");
	return ERROR_NONE;
}
	...
	MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
    ...
    pr_info("[GC8034F] capture setting start\n");
	spin_lock(&imgsensor_drv_lock);
    ...
    pr_info("[GC8034F] capture setting end\n");
	return ERROR_NONE;
}
...
static kal_uint32 normal_video(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
	MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
    pr_info("[GC8034F] normal video setting start\n");
	LOG_INF("E\n");
	LOG_INF("Enter normal_video_setting!\n");
	spin_lock(&imgsensor_drv_lock);
    ...
    pr_info("[GC8034F] normal video setting end\n");
	return ERROR_NONE;
}
...
//[LGE_UPDATE_S][kyunghun.oh@lge.com][2018-9-19] Get FOV from sensor for CTS
//Changed from double pointer to single pointer.
//Reallocation is required in the HAL.
    case SENSOR_FEATURE_GET_SENSOR_VIEWANGLE:
        {
            *((MUINT32 *)(feature_data)) = (0x48)<<16 | (0x48);     //16:9 = 72, 4:3 = 72 // (72)<<16 | (72)
            *(((MUINT32 *)(feature_data))+1) = (0x26)<<16 | (0x26); //16:9 = 38, 4:3 = 38 // (38)<<16 | (38)
            *feature_para_len = sizeof(MUINT32) * 2;
        }
    break;
//[LGE_UPDATE_E][kyunghun.oh@lge.com][2018-9-19] Get FOV from sensor for CTS
	default:
		break;
	}
