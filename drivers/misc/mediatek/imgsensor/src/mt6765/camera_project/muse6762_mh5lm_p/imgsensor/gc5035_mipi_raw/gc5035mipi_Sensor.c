...
#define GC5035_I2C_ADDR		0xA0
extern void m290_get_fusion_id(u8 addr, u8 size, UINT8* fusion_id, u16 i2c_addr);
extern UINT8 fusion_id_front[20];
void gc5035_get_fusion_id(void)
{
//	int i = 0;
	m290_get_fusion_id(0x1D, 10, &fusion_id_front[0], GC5035_I2C_ADDR);
#if 0
	for (i = 0; i < 10; i++) {
		printk("Jasonn fusion_id[%d] = (0x%04x)", i, fusion_id_front[i]);
	}
#endif
}

/*M290 OTP bring up*/
extern kal_uint16 m290_otp_read(otp_format_t *p, u16 i2c_addr, enum OTP_FORM otp_form, UINT8* data_c, u8 check);
...
static void gc5035_otp_read(void)
{
	...
	otp_format_t otp_format_data[M290_OTP_FORM_MAX] = {
		...
		UINT16 check_addr;
		*/
		{true,  0x02F, M290_AWB_SIZE+6, 0, 0x03B},
		{true,  0x03E, M290_AWB_SIZE+6, M290_AWB_SIZE, 0x04A},
		{false,     0, M290_AF_SIZE,  M290_AWB_SIZE + M290_AWB_SIZE, 0},
		{true,  0x04D, M290_LSC_SIZE, M290_AWB_SIZE + M290_AWB_SIZE + M290_AF_SIZE, 0x3C1},
		{false,  0, M290_LSC_SIZE, M290_AWB_SIZE + M290_AWB_SIZE + M290_AF_SIZE + M290_LSC_SIZE, 0}
	};

	printk("%s", __func__);
	for (i = 0; i < M290_OTP_FORM_MAX; i++)
		if (otp_format_data[i].flag)
			m290_otp_read(&otp_format_data[i], GC5035_I2C_ADDR, i, data, 1);

	printk("gc5035_otp_read JJJason lsc_3000k_flag(%d) lsc_5100_flag(%d) af_flag(%d) awb_3000k_flag(%d) awb_5100K_flag(%d) sizeof(%d)", 
			m290_otp_data[1].lsc_3000k_flag, m290_otp_data[1].lsc_5100_flag, m290_otp_data[1].af_flag,
			m290_otp_data[1].awb_3000k_flag, m290_otp_data[1].awb_5100K_flag, M290_OTP_SIZE);
	gc5035_get_fusion_id();
