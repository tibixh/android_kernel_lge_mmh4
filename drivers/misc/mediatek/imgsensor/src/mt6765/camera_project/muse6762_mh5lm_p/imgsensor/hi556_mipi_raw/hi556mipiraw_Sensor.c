 ...
 */
/* updata on 20190408 by sensor vendor
*/
...
/*M290 OTP bring up*/
static void hi556_otp_format_read(otp_format_t *p, enum OTP_FORM otp_form, UINT8* data_c)
{
	...
	UINT8 group = OTP_read_cmos_sensor(start_addr - 1);
	UINT16 checksum_read = 0;
	UINT32 checksum = 0;

	start_addr += ((group & 0x2) + (group & 0x4))*(size+2);
	write_cmos_sensor_byte(0x10a, (start_addr >> 8) & 0xff); //start addr H
	write_cmos_sensor_byte(0x10b, start_addr & 0xff);	//start addr L
	write_cmos_sensor_byte(0x102, 0x01);	//single mode
	for (i = 0; i < size; i++) {
		data[i] = read_cmos_sensor_byte(0x108); //otp data read
		checksum += data[i];
	}
	checksum_read = (read_cmos_sensor_byte(0x108)&0xFF) + ((read_cmos_sensor_byte(0x108)&0xFF) << 8);

	data_c[M290_OTP_SIZE - 2 - otp_form] = ((checksum % 0xFFFF + 1) == checksum_read);
	printk("hi556_otp_format_read otp_form(%d) group(%d) checksum(0x%04x) checksum_read(0x%x) check_ok(%d)",
			otp_form, group, checksum % 0xFFFF + 1, checksum_read, data_c[M290_OTP_SIZE - 2 - otp_form]);
#if 1
	for (i = 0; i < size - 5; i += 6) {
		printk("m290_otp_read HI556 data %d: 0x%02x, 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x", i, data[i], data[i+1], data[i+2], data[i+3], data[i+4], data[i+5]);
	}
	while (i < size) {
		printk("m290_otp_read HI556 data %d: 0x%02x ", i, data[i]);
		i++;
	}
#endif
}

extern UINT8 fusion_id_front[20];
void hi556_get_fusion_id(void)
{
	int i;
	write_cmos_sensor_byte(0x10a, 0x00); //start addr H
	write_cmos_sensor_byte(0x10b, 0x01);	//start addr L
	write_cmos_sensor_byte(0x102, 0x01);	//single mode
	for (i = 0; i < 9; i++) {
		fusion_id_front[i] = read_cmos_sensor_byte(0x108); //otp data read
		//printk("JJason fusion_id_front[%d]:(0x%04x)\n", i, fusion_id_front[i]);
	}
}

extern m290_otg_data_t m290_otp_data[2];
static void HI556_otp_read(void)
{
	UINT8 *data = (UINT8*)&m290_otp_data[1];
	int i;
	otp_format_t otp_format_data[M290_OTP_FORM_MAX] = {
		{true,  0x41E, M290_AWB_SIZE, 0},
		{true,  0x437, M290_AWB_SIZE, M290_AWB_SIZE},
		{false, 0x000, M290_AF_SIZE,  M290_AWB_SIZE + M290_AWB_SIZE},
		{true,  0x450, M290_LSC_SIZE, M290_AWB_SIZE + M290_AWB_SIZE + M290_AF_SIZE},
		{false, 0x000, M290_LSC_SIZE, M290_AWB_SIZE + M290_AWB_SIZE + M290_AF_SIZE + M290_LSC_SIZE}
	};
	...
	for (i = 0; i < M290_OTP_FORM_MAX; i++)
		if (otp_format_data[i].flag)
			hi556_otp_format_read(&otp_format_data[i], i, data);
