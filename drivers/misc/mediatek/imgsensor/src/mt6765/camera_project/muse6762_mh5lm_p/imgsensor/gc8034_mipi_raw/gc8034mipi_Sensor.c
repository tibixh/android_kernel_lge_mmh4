...
#define GC8034_I2C_ADDR		0xA1
extern void m290_get_fusion_id(u8 addr, u8 size, UINT8* fusion_id, u16 i2c_addr);
extern UINT8 fusion_id[20];
void gc8034_get_fusion_id(void)
{
	//int i = 0;
	m290_get_fusion_id(0x20, 9, &fusion_id[0], GC8034_I2C_ADDR);
#if 0
	for (i = 0; i < 9; i++) {
		printk("Jasonn fusion_id[%d] = (0x%04x)", i, fusion_id[i]);
	}
#endif
}
...
static void gc8034_otp_read(void)
{
	UINT8 *data = (UINT8*)&m290_otp_data[0];
	int i;
	otp_format_t otp_format_data[M290_OTP_FORM_MAX] = {
		{true,  0x031, M290_AWB_SIZE, 0, 0x037},
		{true,  0x03A, M290_AWB_SIZE, M290_AWB_SIZE, 0x040},
		{true,  0x731, M290_AF_SIZE,  M290_AWB_SIZE + M290_AWB_SIZE, 0x739},
		{true,  0x043, M290_LSC_SIZE, M290_AWB_SIZE + M290_AWB_SIZE + M290_AF_SIZE, 0x3B7},
		{true,  0x3BA, M290_LSC_SIZE, M290_AWB_SIZE + M290_AWB_SIZE + M290_AF_SIZE + M290_LSC_SIZE, 0x72E}
	};
	...
	for (i = 0; i < M290_OTP_FORM_MAX; i++)
		if (otp_format_data[i].flag)
			m290_otp_read(&otp_format_data[i], GC8034_I2C_ADDR, i, data, 0);

	gc8034_get_fusion_id();
	AFRegulatorCtrl(2);
}
