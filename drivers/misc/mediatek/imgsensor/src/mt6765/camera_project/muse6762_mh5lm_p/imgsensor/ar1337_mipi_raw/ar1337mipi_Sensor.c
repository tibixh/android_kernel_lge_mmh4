...
#define LLPCK_ORIG 0x12BC
#define LLPCK_LONG 0x1A00 

/****************************Modify Following Strings for Debug****************************/
#define PFX "[Murphy]AR1336_camera_sensor"
#define LOG_1 LOG_INF("AR1336,MIPI 4LANE\n")
...
#define LOG_CORN(format, args...)    pr_debug(PFX "[%s] " format, __FUNCTION__, ##args)
...
static imgsensor_info_struct imgsensor_info = {
    ...
    .pre = {
        ...
        .linelength = 4796,                //record different mode's linelength
...
static void set_shutter(kal_uint16 shutter)
{
    ...
    write_cmos_sensor(0x0104, 0x01);
      ...
      if (imgsensor.sensor_mode == IMGSENSOR_MODE_CAPTURE) {
		...
		if(shutter < LEN_H_THRESHOLD) {
		        write_cmos_sensor_16bit(0x0342, 0x12BC);
...
static kal_uint16 set_gain(kal_uint16 gain)
{
	...
	/*reduce powwer noise	start
		if (gain  > 25 * BASEGAIN)
			write_cmos_sensor_16bit(0x3220, 0x0c12);
		else
			write_cmos_sensor_16bit(0x3220, 0x0c13);
		LOG_INF("Vincen_gain = %d , register0x3220 = 0x%x\n ", gain,read_cmos_sensor(0x3220));
		/reduce powwer noise end*/
	if(gain<2*BASEGAIN)
	{
		digital_gain = 64;
...
static void sensor_init(void)
{
	...
	write_cmos_sensor_16bit(0x32D4, 0x0001); // PE_PARAM_VALUE
	write_cmos_sensor_16bit(0x32D4, 0x0000); // PE_PARAM_VALUE
	write_cmos_sensor_16bit(0x3220, 0x0000);  // PDAF_CONTROL
	...
	write_cmos_sensor_16bit(0x32D4, 0x0001);
	write_cmos_sensor_16bit(0x32D4, 0x0000);
	write_cmos_sensor_16bit(0x3220, 0x0000);
...
static void preview_setting(void)
{
	...
	write_cmos_sensor_16bit(0x3040, 0x0041);  // READ_MODE   //0xC041
	...
	write_cmos_sensor_16bit(0x0342, 0x12BC);  // LINE_LENGTH_PCK
...
static kal_uint32 set_test_pattern_mode(kal_bool enable)
{
    ...
    LOG_INF("0x3070=0x%x\n", read_cmos_sensor(0x3070));
    spin_lock(&imgsensor_drv_lock);
    imgsensor.test_pattern = enable;
    spin_unlock(&imgsensor_drv_lock);
	LOG_INF("[test_pattern]Exit!!!");
    return ERROR_NONE;
}
...
#ifdef CONFIG_HQ_HARDWARE_INFO
static void print_pdafData(void)
{
#if DEBUG
	LOG_INF("%s:[Murphy] printf 4 data pdaf1 [0x%04x] = [0x%02x],[0x%04x] = [0x%02x] ,[0x%04x] = [0x%02x],[0x%04x] = [0x%02x]",
				__func__, 0x0E2D, pOtp_data.pdaf[0], 0x0E2E,pOtp_data.pdaf[1],
				0x0E2F,pOtp_data.pdaf[2], 0x0E30,pOtp_data.pdaf[3]);
	LOG_INF("%s: Murphy [0x%04x] = [0x%02x],[0x%02x],[0x%02x],[0x%02x]",
				__func__, 0x0ECF, pOtp_data.pdaf_cali[0], pOtp_data.pdaf_cali[1],
				pOtp_data.pdaf_cali[2],  pOtp_data.pdaf_cali[3]);
#endif

	return ;
}
static void read_otp_pdaf_data(EEPROM_DATA *data)
{
	kal_uint32 i = 0;

#if DEBUG
	for (i = 0x0E2D; i <= 0x0ECE; i++)
		LOG_INF("%s :[Murphy][0x%04x] = [0x%02x]\n", __func__, i, read_eeprom_module(i));
#endif

	for (i = 0x0; i< 162; i++)
		data->pdaf[i] = read_eeprom_module((i + 0x0E2D));

	for (i = 0x0; i < 4; i++)
		data->pdaf_cali[i] = read_eeprom_module((i + 0x0ECF));
...
static void write_otp_pdaf_data(void)
{
	kal_uint32 j = 0;
	kal_uint32 i = 0;

    for (j=0x3232,i=0; j<=0x32B8; j+=2,i+=2)
        write_cmos_sensor_16bit(j, ((pOtp_data.pdaf[i+1]<<8)|pOtp_data.pdaf[i]));
    write_cmos_sensor_16bit(0x3C92, ((pOtp_data.pdaf[i+1]<<8)|pOtp_data.pdaf[i]));
    i += 2;
    j = 0x3CA8;
    for(;j<=0x3CBE; j+=2) {
        write_cmos_sensor_16bit(j, ((pOtp_data.pdaf[i+1]<<8)|pOtp_data.pdaf[i]));
	...
	for (j=0x3232; j<=0x3250; j+=2)
		LOG_INF("%s :[Murphy][0x%04x] = [0x%04x]\n", __func__, j, read_cmos_sensor_16bit(j));
                      ...
                      MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
    ...
    imgsensor.autoflicker_en = KAL_FALSE;
    spin_unlock(&imgsensor_drv_lock);
	capture_setting(imgsensor.current_fps);//[LGE_UPDATE] [bk.bae@lge.com] [2019-02-19] make the fullsize recording works
	//set_mirror_flip(sensor_config_data->SensorImageMirror);
		//set_mirror_flip(imgsensor.mirror);
    return ERROR_NONE;
}    /*    normal_video   */
...
UINT32 AR1337_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
	...
	printk("AR1337_MIPI_RAW_SensorInit in\n");
    if (pfFunc!=NULL)
        *pfFunc=&sensor_func;
    return ERROR_NONE;
