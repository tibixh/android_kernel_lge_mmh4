...
#define HI846_I2C_ADDR		0xB1
extern void m290_get_fusion_id(u8 addr, u8 size, UINT8* fusion_id, u16 i2c_addr);
extern UINT8 fusion_id[20];
void hi846_get_fusion_id(void)
{
//	int i = 0;
	m290_get_fusion_id(0x1D, 10, &fusion_id[0], HI846_I2C_ADDR);
#if 0
	for (i = 0; i < 10; i++) {
		printk("fusion_id[%d] = (0x%04x)", i, fusion_id[i]);
	}
#endif
}
...
static void hi846_otp_read(void)
{
	...
	otp_format_t otp_format_data[M290_OTP_FORM_MAX] = {
		...
		UINT16 check_addr;
		*/
		{true,  0x03A, M290_AWB_SIZE, 0, 0x046},
		{true,  0x049, M290_AWB_SIZE, M290_AWB_SIZE, 0x055},
		{true,  0x02F, M290_AF_SIZE,  M290_AWB_SIZE + M290_AWB_SIZE, 0x37},
		{true,  0x058, M290_LSC_SIZE, M290_AWB_SIZE + M290_AWB_SIZE + M290_AF_SIZE, 0x3CC},
		{true,  0x3CF, M290_LSC_SIZE, M290_AWB_SIZE + M290_AWB_SIZE + M290_AF_SIZE + M290_LSC_SIZE, 0x743}
	};
	...
	for (i = 0; i < M290_OTP_FORM_MAX; i++)
		if (otp_format_data[i].flag)
			m290_otp_read(&otp_format_data[i], HI846_I2C_ADDR, i, data, 1);
