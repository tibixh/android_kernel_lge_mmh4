...
#define PFX "[Murphy]s5k5e9yx_camera_sensor"
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
				...
				#endif
				pr_debug("[s5k5e9]i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);	  
				return ERROR_NONE;
			}	
			retry--;
		} while(retry > 0);
...
static kal_uint32 set_test_pattern_mode(kal_bool enable)
{
	...
	if (enable) {
		...
		write_cmos_sensor(0x3462, 0x00);
		write_cmos_sensor(0x3230, 0x01);
		write_cmos_sensor(0x3290, 0x01);
		write_cmos_sensor(0x3201, 0x01);
		write_cmos_sensor(0x0b05, 0x00);
		write_cmos_sensor(0x0b00, 0x00);
		write_cmos_sensor(0x3c60, 0x00);
		write_cmos_sensor(0x0601, 0x02);
	} else {
		write_cmos_sensor(0x3200, 0x01);
		write_cmos_sensor(0x3462, 0x03);
		write_cmos_sensor(0x3230, 0x00);
		write_cmos_sensor(0x3290, 0x10);
		write_cmos_sensor(0x3201, 0x18);
		write_cmos_sensor(0x0b05, 0x01);
		write_cmos_sensor(0x0b00, 0x00);
		write_cmos_sensor(0x3c60, 0x01);
...
UINT32 S5K5E9_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
	...
	printk("S5K5E9_MIPI_RAW_SensorInit in\n");
	if (pfFunc!=NULL)
		*pfFunc=&sensor_func;
	return ERROR_NONE;
