...
static void normal_video_setting(kal_uint16 currefps)
{
	...
	LOG_DBG("E! video just has 30fps preview size setting ,NOT HAS 24FPS SETTING!\n");
	preview_setting();
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = return_sensor_id();
			/* lx_revised */
			LOG_DBG("[ov16885]Read sensor id OK, write id:0x%x ,sensor Id:0x%x\n",
			...
			LOG_DBG("Read sensor id OK:0x%x, id: 0x%x\n", imgsensor.i2c_write_id,
				*sensor_id);
			retry--;
		} while (retry > 0);
		i++;
	...
	LOG_DBG("park Read sensor sensor_id fail, id: 0x%x\n", *sensor_id);
	return ERROR_NONE;
}
...
static kal_uint32 open(void)
{
	...
	LOG_DBG("PLATFORM:MIPI 4LANE ov16885 open+++++ ++++\n");
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			sensor_id = return_sensor_id();
			if (sensor_id == imgsensor_info.sensor_id) {
				LOG_DBG("[16885]i2c write id: 0x%x, sensor id: 0x%x\n",
					imgsensor.i2c_write_id, sensor_id);
				break;
			}
			LOG_DBG("Read sensor id fail, id: 0x%x,sensor_id =0x%x\n",
				imgsensor.i2c_write_id, sensor_id);
			retry--;
		} while (retry > 0);
		i++;
