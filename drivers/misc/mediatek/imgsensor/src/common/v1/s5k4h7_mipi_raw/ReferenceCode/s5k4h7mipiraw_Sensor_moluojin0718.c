...
static kal_uint32 s5k4h7_otp_apply(struct s5k4h7_otp_data_str *pData)
{
	...
	if ((pData->otp_ready == 1) && (pData->data_valid == 1)) {
		...
		LOG_DBG("Before apply AWB: r_gain = 0x%x, b_gain = 0x%x, gr_gain = 0x%x, gb_gain = 0x%x\n",
				(read_cmos_sensor_8(0x0210) & 0xFF) | (read_cmos_sensor_8(0x0211) << 8),
				(read_cmos_sensor_8(0x0212) & 0xFF) | (read_cmos_sensor_8(0x0213) << 8),
				(read_cmos_sensor_8(0x020E) & 0xFF) | (read_cmos_sensor_8(0x020F) << 8),
				(read_cmos_sensor_8(0x0214) & 0xFF) | (read_cmos_sensor_8(0x0215) << 8));
...
static void s5k4h7_get_otp(void)
{
	...
	if (pOTP_Data.otp_ready != 1 || pOTP_Data.data_valid != 1) {
		...
		if (pOTP_Data.info_checksum == pOTP_Data.checksum_calc) {
