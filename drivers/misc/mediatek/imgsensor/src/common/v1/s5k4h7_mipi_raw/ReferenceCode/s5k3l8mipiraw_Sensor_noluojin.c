...
//wei.miao@reallytek.com 20160918@modify for camera performance begin
#ifdef PERFORMANCE_IMPROVE
...
#endif
//wei.miao@reallytek.com 20160918@modify for camera performance end
	...
	.isp_driving_current = ISP_DRIVING_4MA, //mclk driving current  //miaowei add 8->4 for resole RF
...
//flag value
int S5K3L8_read_otp(struct S5K3L8_MIPI_otp_struct *otp_ptr)
{
   int otp_flag,addr,temp,i;
	 addr=0;
	 i=0;

      write_cmos_sensor(0x0136, 0x1800);//External CLK 24MHz......Follow setting according to diff CLK
     ...
     mdelay(10);
	 write_cmos_sensor_byte(0x0A02,0x01);//set 1page 
	 write_cmos_sensor_byte(0x0A00,0x01);//set start read addr
	 
	 otp_flag = read_cmos_sensor(0x0A04);
	 LOG_INF("-----hejun ofilm S5K3L8_read_otp() otp_flag=%x\n",otp_flag);
	 if((otp_flag&0xC0)==0x40)         // if((otp_flag&0x40)==0x40)   hejun-0701
	 {
	    addr=0x0A05; //group 1
	    i=1;
	 }
	 else if((otp_flag&0x30)==0x10)     //else if((otp_flag&0x10)==0x10)  hejun-0701
	 {
	    addr=0x0A16; //group 2
	    i=2;
	 }
     ...
     (*otp_ptr).rg_ratio = (((read_cmos_sensor(temp) << 8)&0x0300) + (read_cmos_sensor(0x0A0F)));
     (*otp_ptr).bg_ratio = (((read_cmos_sensor(temp) << 6)&0x0300) + (read_cmos_sensor(0x0A10)));	 
     }
     else if(i==2)
     {
     temp = 0x0A24;
     //OTP WB Calibration Data
     (*otp_ptr).rg_ratio = (((read_cmos_sensor(temp) << 8)&0x0300) + (read_cmos_sensor(0x0A20)));
     (*otp_ptr).bg_ratio = (((read_cmos_sensor(temp) << 6)&0x0300) + (read_cmos_sensor(0x0A21)));	      
     }  
     
	 LOG_INF("----hejun ofilm S5K3L8_read_otp() otp_flag=%x\n",otp_flag);
	 LOG_INF("----hejun ofilm S5K3L8_read_otp() Module_integrator_id=%x\n",(*otp_ptr).Module_integrator_id);
	 LOG_INF("----hejun ofilm S5K3L8_read_otp() rg_ratio=%x\n",(*otp_ptr).rg_ratio);
	 LOG_INF("----hejun ofilm S5K3L8_read_otp() bg_ratio=%x\n",(*otp_ptr).bg_ratio);
	 
	  //write_cmos_sensor_byte(0x0A00,0x00);//set read end 
	//  write_cmos_sensor_byte(0x0100,0x00); //stream off
	//write_cmos_sensor_byte(0x0b00,0x01);   //shading on by hejun-0701
...
#define S5K3L8_RG_Ratio_Typical  0x24D
#define S5K3L8_BG_Ratio_Typical  0x265 	
int S5K3L8_apply_otp(void)
{
  ...
  if(current_otp.flag==0xC0)
  {
    rg=current_otp.rg_ratio;
    bg=current_otp.bg_ratio;
    
		r_ratio = 512 * (S5K3L8_RG_Ratio_Typical) /(rg);
		b_ratio = 512 * (S5K3L8_BG_Ratio_Typical) /(bg);
		...
		//write_cmos_sensor(0x0B00,0x0001);  //lsc on
	
		//write_cmos_sensor(0x0100,0x0001); //stream on
		//mdelay(100);	
		
		LOG_INF("----hejun ofilm S5K3L8_apply_otp() R_GAIN=%x,B_GAIN=%x,G_GAIN=%x\n",R_GAIN,B_GAIN,G_GAIN);
...
//	write_cmos_sensor(0x3442, 0x0100);	//new setting
	...
	write_cmos_sensor(0x3058, 0x0100);	// updated by reference V151030
	write_cmos_sensor(0x6028, 0x2000);
	...
	write_cmos_sensor(0x6028, 0x4000);	// updated 0501 by reference V151030
	write_cmos_sensor(0x1124, 0x4100);	// updated 0501 by reference V151030
	write_cmos_sensor(0x1126, 0x0000);	// updated 0501 by reference V151030
	write_cmos_sensor(0x112C, 0x4100);	// updated 0501 by reference V151030
	write_cmos_sensor(0x112E, 0x0000);	// updated 0501 by reference V151030
	write_cmos_sensor(0x3442, 0x0100);	// updated 0501 by reference V151030
	
//add by hejun LSC Control
	write_cmos_sensor(0x6028, 0x2000);
	write_cmos_sensor(0x602A, 0x0CF4);
	write_cmos_sensor(0x6F12, 0x0140);	 //for OTP verification
	write_cmos_sensor(0x6028, 0x4000);
	write_cmos_sensor(0x602A, 0x0B00);
	write_cmos_sensor(0x6F12, 0x0107);	 // 01:lsc on,00:lsc off
//add
...
//$MV1[MCLK:24,Width:2104,Height:1560,Format:MIPI_Raw10,mipi_lane:4,mipi_datarate:1124,pvi_pclk_inverse:0]
  ...
  write_cmos_sensor(0x0304, 0x0006);
	write_cmos_sensor(0x0306, 0x00B1);	// updated 0501 by reference V151030
  write_cmos_sensor(0x0302, 0x0001);
  write_cmos_sensor(0x0300, 0x0005);
  write_cmos_sensor(0x030C, 0x0006);
...
static void capture_setting(kal_uint16 currefps)
{
  ...
  write_cmos_sensor(0x0136, 0x1800);
  write_cmos_sensor(0x0304, 0x0006);
		write_cmos_sensor(0x0306, 0x00B1);	// updated by reference V151030
  write_cmos_sensor(0x0302, 0x0001);
  write_cmos_sensor(0x0300, 0x0005);
  write_cmos_sensor(0x030C, 0x0006);
  write_cmos_sensor(0x030E, 0x0119);
  ...
  write_cmos_sensor(0x0342, 0x16B0);
  #endif
		write_cmos_sensor(0x0340, 0x0CA2);	// updated by reference V151030
  write_cmos_sensor(0x0202, 0x0200);
  write_cmos_sensor(0x0200, 0x00C6);
  write_cmos_sensor(0x0B04, 0x0101);
  write_cmos_sensor(0x0B08, 0x0000);
...
static void hs_video_setting(void)
{
  ...
  write_cmos_sensor(0x0342, 0x16B0);
  #endif
	write_cmos_sensor(0x0340, 0x032C);	// updated by reference V151030
  write_cmos_sensor(0x0202, 0x0200);
  write_cmos_sensor(0x0200, 0x00C6);
  write_cmos_sensor(0x0B04, 0x0101);
  write_cmos_sensor(0x0B08, 0x0000);
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		//read_imx135_otp_mtk_fmt();
				...
				LOG_INF("i2c write id: 0x%x, ReadOut sensor id: 0x%x, imgsensor_info.sensor_id:0x%x.\n", imgsensor.i2c_write_id,*sensor_id,imgsensor_info.sensor_id);
		 		app_get_front_sensor_name("s5k3l8");
                return ERROR_NONE;
            }
