...
//#define S5K4H7_OTP_CALI
#undef S5K4H7_OTP_CALI
...
static struct imgsensor_info_struct imgsensor_info = {
	...
	.cap = {
		.pclk = 280000000,				//record different mode's pclk
		.linelength  = 3688,//5808,				//record different mode's linelength
		.framelength = 2530,			//record different mode's framelength
		.startx = 0,					//record different mode's startx of grabwindow
		.starty = 0,					//record different mode's starty of grabwindow
...
#ifdef S5K4H7_OTP_CALI
static struct s5k4h7_otp_data_str pOTP_Data = {0};

static inline void otp_read_enable(void)
{
	kal_uint32 j = 0;

	write_cmos_sensor_8(0x0100, 0x01);//Stream On
	for (j = 0; j < 64; j++) {//Clear buffers
		write_cmos_sensor_8(0x0A04, 0x00);
	}
	write_cmos_sensor_8(0x0A02, 21);//Set Page: Page 21 for INFO & AWB
	write_cmos_sensor_8(0x0A00, 0x01);//OTP read enable
	mdelay(50);
}

static inline void otp_read_disable(void)
{
	write_cmos_sensor_8(0x0A00, 0x01);//OTP read disable
}

static kal_uint32 s5k4h7_otp_read(struct s5k4h7_otp_data_str *pData, kal_uint16 start_addr)
{
	kal_uint16 i = 0;
	kal_uint32 checksum = 0;
	u8 *p = (u8 *)pData;

	for (i = 0; i < sizeof(struct s5k4h7_otp_data_str) / sizeof(kal_uint8) - 3; i++) {
		*p = read_cmos_sensor_8(start_addr + i);
		checksum += *p;
		//LOG_INF("reg[0x%x]: 0x%x\n", i, *p);
		p++;
	}
	pData->checksum_calc = (checksum - pData->info_checksum) % 255 + 1;
	LOG_DBG("checksum_calc = 0x%x, checksum = 0x%x\n",
			pData->checksum_calc, pData->info_checksum);

	return i;
}

#define GAIN_DEFAULT 0x0100
static kal_uint32 s5k4h7_otp_apply(struct s5k4h7_otp_data_str *pData)
{
	/* Apply OTP AWB here*/
	kal_uint32 rg, bg, gr_gb, r_ratio, b_ratio,
			   R_GAIN, B_GAIN, G_GAIN, Gb_GAIN, Gr_GAIN,
			   rg_typical, bg_typical, gr_gb_typical;

	if ((pData->otp_ready == 1) && (pData->data_valid == 1)) {
		rg = (pData->R_over_G_unit_h << 8) | (pData->R_over_G_unit_l & 0xFF);
		bg = (pData->B_over_G_unit_h << 8) | (pData->B_over_G_unit_l & 0xFF);
		gr_gb = (pData->Gr_over_Gb_unit_h << 8) | (pData->Gr_over_Gb_unit_l & 0xFF);
		LOG_DBG("rg = 0x%x, bg = 0x%x, gr_gb = 0x%x\n", rg, bg, gr_gb);

		rg_typical = (pData->R_over_G_golden_h << 8) | (pData->R_over_G_golden_l & 0xFF);
		bg_typical = (pData->B_over_G_golden_h << 8) | (pData->B_over_G_golden_l & 0xFF);
		gr_gb_typical = (pData->Gr_over_Gb_golden_h << 8) | (pData->Gr_over_Gb_golden_l & 0xFF);
		LOG_DBG("rg_typical = 0x%x, bg_typical = 0x%x, gr_gb_typical = 0x%x\n",
				rg_typical, bg_typical, gr_gb_typical);

		r_ratio = 1024 * (rg_typical) / rg;
		b_ratio = 1024 * (bg_typical) / bg;
		...
		if (r_ratio >= 1024) {
			...
			if (b_ratio >= 1024) {
				...
				Gr_GAIN = GAIN_DEFAULT * 1024 / r_ratio;
				Gb_GAIN = GAIN_DEFAULT * 1024 / b_ratio;
		...
		LOG_DBG("R_GAIN: 0x%x, B_GAIN: 0x%x, G_GAIN: 0x%x, G_GAIN: 0x%x\n", R_GAIN, B_GAIN, G_GAIN, G_GAIN);

		LOG_DBG("Before apply AWB: r_gain = 0x%x, b_gain = 0x%x, gr_gain = 0x%x, gb_gain = 0x%x\n",
				read_cmos_sensor(0x0210), read_cmos_sensor(0x0212),
				read_cmos_sensor(0x020E), read_cmos_sensor(0x0214));
		...
		LOG_DBG("After apply AWB: r_gain = 0x%x, b_gain = 0x%x, gr_gain = 0x%x, gb_gain = 0x%x\n",
				read_cmos_sensor(0x0210), read_cmos_sensor(0x0212),
				read_cmos_sensor(0x020E), read_cmos_sensor(0x0214));
	}
...
static void s5k4h7_get_otp(void)
{
	kal_uint8 info_flag = 0;
	kal_uint16 info_start_addr = 0;

	if (pOTP_Data.otp_ready != 1 || pOTP_Data.data_valid != 1) {
		/* Get OTP data before return no err */
		otp_read_enable();//Enable OTP read
		info_flag = read_cmos_sensor_8(0x0A04);
		LOG_INF("info_flag = 0x%x", info_flag);
		if ((info_flag >> 6) == 0x01) {//Group 1
			info_start_addr = 0x0A05;
		} else if (((info_flag >> 4) && 0x03) == 0x01) {//Group 2
			info_start_addr = 0x0A25;
		} else {
			LOG_INF("No valid Info");
			info_start_addr = 0;
		}

		if (info_start_addr != 0)
			s5k4h7_otp_read(&pOTP_Data, info_start_addr);

		otp_read_disable();//Disable OTP read

		if ((info_flag != 0) && (pOTP_Data.info_checksum == pOTP_Data.checksum_calc)) {
			LOG_INF("Checksum Success\n");
			pOTP_Data.data_valid = 1;
			pOTP_Data.otp_ready = 1;
		} else {
			LOG_INF("Checksum Fail\n");
			pOTP_Data.data_valid = 0;
			pOTP_Data.otp_ready = 0;
		}
	}
	LOG_DBG("checksum_calc = 0x%x, checksum = 0x%x, data_valid = 0x%x, otp_ready = 0x%x\n",
			pOTP_Data.checksum_calc, pOTP_Data.info_checksum, pOTP_Data.data_valid, pOTP_Data.otp_ready);
}
#endif
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = return_sensor_id();
			pr_err("[Murphy]sensor_id = 0x%x imgsensor_info.sensor_id= 0x%x", *sensor_id, imgsensor_info.sensor_id);
			if (*sensor_id == imgsensor_info.sensor_id) {

#ifdef S5K4H7_OTP_CALI
				s5k4h7_get_otp();
				LOG_DBG("i2c write id: 0x%x, sensor id: 0x%x vendorID=0x%x\n",
							imgsensor.i2c_write_id, *sensor_id,pOTP_Data.supplier_code);
				//zaki add ++++++++++
				if(pOTP_Data.supplier_code==0x56){
					hw_info_sub_otp.sensor_name = SENSOR_DRVNAME_S5K4H7_MIPI_RAW;
					return ERROR_NONE;
				}
				else
				{
					pOTP_Data.supplier_code = 0;
					pOTP_Data.data_valid = 0;
					pOTP_Data.otp_ready = 0;
				...
				//zaki add -------------
#endif
					pr_err("[Murphy][s5k4h7]i2c write id: 0x%x, sensor id: 0x%x \n",
...
#ifdef S5K4H7_OTP_CALI
	/** 
	 * Retry read OTP if got invalid data in sensor search 
	 * else apply OTP AWB & LSC here
	 **/
	if ((pOTP_Data.otp_ready != 1) || (pOTP_Data.data_valid != 1))
		s5k4h7_get_otp();
	if ((pOTP_Data.otp_ready == 1) && (pOTP_Data.data_valid == 1))
		s5k4h7_otp_apply(&pOTP_Data);
#endif
