...
u32 get_efuse_values(u32 *result_verify, u32 *result_part)
{
	...
	u32 temp_same = 0, temp_mtk = 0, temp_lge = 0, temp_huaqin = 0;
...
// ---------------------------------
	...
	for (i = 0; i < ARRAY_SIZE(pubk_list); i++) {
		...
		if (value_part == pubk_list[i].lge_default_key)
			temp_lge |= (0x1 << i);
		if (value_part == pubk_list[i].huaqin_default_key)
			temp_huaqin |= (0x1 << i);
	}
	...
	if ( *result_part == 0x1E || *result_part == 0x3E ) {
		*result_part |= 0x1;
		fuse_value = read_efuse_memory(SBC_KERNEL_INDEX);
		fuse_value &= SBC_KERNEL_MASKING;
		if ( fuse_value > 0 && (*result_verify == 0x1E || *result_verify == 0x3E))
			*result_verify |= 0x1;
	}
