...
#define APMIXED_REG(ofs)    (apmixed_base+ofs)
#define MCUCFG_REG(ofs)     (mcucfg_base+ofs)
#define CCIPLL_CON1         APMIXED_REG(0x230)
#define CKDIV1_CCI_CFG      MCUCFG_REG(0x7C0)
#define CCI_CLK_CTR         MCUCFG_REG(0x660)
#define MP_GIC_RGU_SYNC_DCM MCUCFG_REG(0x758)
...
static void gic_do_wait_for_rwp(void __iomem *base)
{
	...
	while (readl_relaxed(base + GICD_CTLR) & GICD_CTLR_RWP) {
		...
		if (!count) {
			...
			pr_err_ratelimited("CCIPLL_CON1     :0x%08x\n",
				readl_relaxed(CCIPLL_CON1));
			pr_err_ratelimited("CKDIV1_CCI_CFG  :0x%08x\n",
				readl_relaxed(CKDIV1_CCI_CFG));
			pr_err_ratelimited("CCI_CLK_CTR     :0x%08x\n",
				readl_relaxed(CCI_CLK_CTR));
			pr_err_ratelimited("MP_GIC_RGU_SYNC_DCM :0x%08x\n",
				readl_relaxed(MP_GIC_RGU_SYNC_DCM));
			BUG();
			return;
		}
		cpu_relax();
				 ...
				 struct fwnode_handle *handle)
{
	...
	err = get_base_from_node("mediatek,apmixed", &apmixed_base, 0);
	if (err)
		goto out_free;
	err = get_base_from_node("mediatek,mcucfg", &mcucfg_base, 0);
	if (err)
		goto out_free;
