...
#ifdef CONFIG_LGE_PM
int charger_dev_input_suspend(struct charger_device *chg_dev, bool en)
{
	if (chg_dev != NULL && chg_dev->ops != NULL &&
	    chg_dev->ops->input_suspend)
		return chg_dev->ops->input_suspend(chg_dev, en);

	return -ENOTSUPP;
}
EXPORT_SYMBOL(charger_dev_input_suspend);

int charger_dev_is_input_suspended(struct charger_device *chg_dev, bool *en)
{
	if (chg_dev != NULL && chg_dev->ops != NULL &&
	    chg_dev->ops->is_input_suspended)
		return chg_dev->ops->is_input_suspended(chg_dev, en);
