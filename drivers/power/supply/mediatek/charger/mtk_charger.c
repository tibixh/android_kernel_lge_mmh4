...
bool is_typec_adapter(struct charger_manager *info)
{
	...
	if (tcpm_inquire_typec_remote_rp_curr(info->tcpc) == 0)
		return false;
				...
				struct device *dev)
{
	...
	if (of_property_read_u32(np, "typec_rp3p0_input_current", &val) >= 0) {
		...
		info->data.typec_rp3p0_input_current = TYPEC_RP3P0_INPUT_CURRENT;
	}
	...
	if (of_property_read_u32(np, "typec_rp1p5_input_current", &val) >= 0) {
		...
		info->data.typec_rp1p5_input_current = TYPEC_RP1P5_INPUT_CURRENT;
	}
