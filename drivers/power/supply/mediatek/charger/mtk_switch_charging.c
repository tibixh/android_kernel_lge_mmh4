...
static void swchg_select_charging_current_limit(struct charger_manager *info)
{
	...
	if (mtk_is_TA_support_pd_pps(info)) {
		...
		if (tcpm_inquire_typec_remote_rp_curr(info->tcpc) == 3000) {
			pdata->input_current_limit = info->data.typec_rp3p0_input_current;
			pdata->charging_current_limit = info->data.typec_rp3p0_current;
		} else if (tcpm_inquire_typec_remote_rp_curr(info->tcpc)
			   == 1500) {
			pdata->input_current_limit = info->data.typec_rp1p5_input_current;
			pdata->charging_current_limit = info->data.typec_rp1p5_current;
		} else {
