...
#define BATTERY_REMOVE_CHECK_FILE BATTERY_CYCLE_PATH "battery_remove_check"
#define CBC_BATTERY_COUNT_BACKUP_FILE BATTERY_CYCLE_PATH "cbc_cycle_backup"
#endif
...
void battery_cycle_update(int soc)
{
	...
	if (count) {
		...
		cycle_write_to_fs(BATTERY_COUNT_FILE, battery_cycle_count);
#ifdef CONFIG_LGE_PM_BATTERY_AGING_FACTOR
		cycle_write_to_fs(BATTERY_COUNT_BACKUP_FILE, battery_cycle_count);
#endif
	}
...
static void cycle_clear(void)
{
	...
	cycle_write_to_fs(CBC_BATTERY_COUNT_BACKUP_FILE, 0);
#endif
