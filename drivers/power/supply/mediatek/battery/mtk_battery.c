...
#ifdef CONFIG_LGE_PM_VTS
	bat_data->VTS_batt_temp =
			mtk_thermal_get_temp(LGE_THERMAL_SENSOR_BATTERY);
#endif
}
...
#ifdef CONFIG_LGE_PM_VTS
		if (data->VTS_batt_temp != -127000)
			val->intval = data->VTS_batt_temp;
#endif
#ifdef CONFIG_LGE_PM_BATTERY_PRESENT
		if (!data->BAT_PRESENT)
			val->intval = 200;
...
#endif
#ifdef CONFIG_LGE_PM_VTS
	battery_main.VTS_batt_temp = -127000;
#endif
}
