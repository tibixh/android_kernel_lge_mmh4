...
#ifdef AW8898_ANA_VOL_CONTROL
#define AW8898_BIT_SPK_GAIN_MASK (~(7< 4))
#define AW8898_SPK_GAIN_MAX (7)
#define AW8898_SPK_GAIN_MIN (0)
#define AW8898_SPK_GAIN_REG_SHIFT (4)
static int aw8898_ana_vol = 4;  //Default 14dB
#endif
...
#ifdef AW8898_ANA_VOL_CONTROL
 static const char * const analog_volume_texts[] = { "8dB", "10dB", "12dB", "13dB", "14dB", "15dB", "16dB", "16dB" };
#endif
...
#ifdef CONFIG_LGE_SPK_EXT_DAC_MUTE_CONTROL
static const struct soc_enum SpeakerMute_Enum = SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(spk_function), spk_function);
static unsigned int AW8898_DIGITAL_GAIN_INDEX = 0;
static int SPK_MUTE_FLAG = 0;
static int Speaker_Mute_Switch_get(struct snd_kcontrol *kcontrol,
			    struct snd_ctl_elem_value *ucontrol)
{
    ucontrol->value.integer.value[0] =SPK_MUTE_FLAG;
    return 0;
}
static int Speaker_Mute_Switch_set(struct snd_kcontrol *kcontrol,
			    struct snd_ctl_elem_value *ucontrol)
{
    ...
    if (ucontrol->value.integer.value[0] == 0 || ucontrol->value.integer.value[0] == 1) {
        SPK_MUTE_FLAG = ucontrol->value.integer.value[0];
    } else {
    ...
    if (SPK_MUTE_FLAG == 1) {
		value = (unsigned int)255;
    } else {
		value = AW8898_DIGITAL_GAIN_INDEX;
    }

    pr_info("%s(), SPK_MUTE_FLAG = %d, value = %d\n", __func__, SPK_MUTE_FLAG, value);
    ...
    //cal real value
    value = value << AW8898_VOL_REG_SHIFT&AW8898_BIT_HAGCCFG7_VOL_MASK;
...
#ifdef CONFIG_LGE_SPK_EXT_DAC_MUTE_CONTROL
    AW8898_DIGITAL_GAIN_INDEX = value;
    if(unlikely(SPK_MUTE_FLAG == 1)) {
        pr_info("%s(), mute aw8898 digital gain\n", __func__);
        value = (unsigned int)255;
    }
#endif
...
#ifdef AW8898_ANA_VOL_CONTROL
static int aw8898_ana_vol_get(struct snd_kcontrol *kcontrol,
                               struct snd_ctl_elem_value *ucontrol)
{
    ucontrol->value.integer.value[0] = aw8898_ana_vol;
    return 0;
}

static int aw8898_ana_vol_set(struct snd_kcontrol *kcontrol,
                               struct snd_ctl_elem_value *ucontrol)
{
    struct snd_soc_codec *codec = snd_soc_kcontrol_codec(kcontrol);
    struct aw8898 *aw8898 = snd_soc_codec_get_drvdata(codec);
    unsigned int value = 0, temp_value = 0;
    unsigned int reg_value = 0;

    //value is right
    value = ucontrol->value.integer.value[0];
    temp_value = value;
    if(value > AW8898_SPK_GAIN_MAX || value < AW8898_SPK_GAIN_MIN){
      pr_err("%s:value over range \n",__func__);
      return -1;
    }
    ...
    //cal real value
    value = value << AW8898_SPK_GAIN_REG_SHIFT&AW8898_BIT_SPK_GAIN_MASK;
    aw8898_i2c_read(aw8898, AW8898_REG_CDACTRL1, &reg_value);
    value = value | (reg_value&0xff8f);

    //write value
    aw8898_i2c_write(aw8898, AW8898_REG_CDACTRL1, value);
    aw8898_ana_vol = temp_value;
...
#ifdef AW8898_ANA_VOL_CONTROL
    SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(analog_volume_texts), analog_volume_texts),
#endif
};
...
#ifdef AW8898_ANA_VOL_CONTROL
    SOC_ENUM_EXT("aw8898_anaolg_volume", aw8898_snd_enum[2],
            aw8898_ana_vol_get, aw8898_ana_vol_set),
#endif
#ifdef CONFIG_LGE_SPK_EXT_DAC_MUTE_CONTROL
    SOC_ENUM_EXT("Speaker_Mute_Switch", SpeakerMute_Enum, Speaker_Mute_Switch_get, Speaker_Mute_Switch_set),
#endif
};
