...
static int fs1603_config_pll(fsm_dev_t *fsm_dev, int on)
{
    ...
    if (idx >= size) {
        pr_err("Not found bclk: %d, rate: %d\n", cfg->bclk, cfg->srate);
        return -EINVAL;
    }
...
// LGE I2S Channel selection init Start
    if (fsm_dev->i2c_addr == FSM_DEV_LEFT)
        i2sctrl.bit.CHS12 = FSM_CHN_LEFT;
    else if (fsm_dev->i2c_addr == FSM_DEV_RIGHT)
        i2sctrl.bit.CHS12 = FSM_CHN_RIGHT;
    else
        i2sctrl.bit.CHS12 = FSM_CHN_MONO;

    pr_info("%s i2c_addr : 0x%02x, CHS12: %d, val: 0x%04x\n", __func__, fsm_dev->i2c_addr, i2sctrl.bit.CHS12, i2sctrl.value);
//   LGE I2S Channel selection init End
...
int fs1603_set_mute(fsm_dev_t *fsm_dev, int mute, uint8_t volume)
{
    ...
    if (mute == FSM_UNMUTE) {
        if (fsm_dev->flag & FSM_FLAGS_BYPASS && (fsm_dev->cur_scene == FSM_SCENE_EARPIECE)) {
            ...
            volume = 0xdf;// 0xf8 = -2.625dB, 0xdf = -12db
        }
