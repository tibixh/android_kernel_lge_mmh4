 ...
 *  J.Wakasugi          18/05/10        2.0           01  // for AK4376A & 4.9.XX 
 * ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
 *  kernel version: 4.9
 *
...
static struct hifi_gpio_attr hifi_gpios[PINCTRL_HIFI_NUM] = {
        ...
        [PINCTRL_PDN_OFF] = {"hifi_pdn_off", NULL},
        [PINCTRL_PDN_ON] = {"hifi_pdn_on", NULL},
        [PINCTRL_POWER_LDO_OFF] = {"hifi_ldo_off", NULL},
        [PINCTRL_POWER_LDO_ON] = {"hifi_ldo_on", NULL},
        [PINCTRL_SW_OFF] = {"hifi_sw_off", NULL},
        [PINCTRL_SW_ON] = {"hifi_sw_on", NULL},
};
...
#endif
		if(hifi_gpios[PINCTRL_SW_ON].gpioctrl != NULL)
			pinctrl_select_state(pinctrl_hifi, hifi_gpios[PINCTRL_SW_ON].gpioctrl);

		if(hifi_gpios[PINCTRL_PDN_ON].gpioctrl != NULL)
			pinctrl_select_state(pinctrl_hifi, hifi_gpios[PINCTRL_PDN_ON].gpioctrl);
...
#endif
		if(hifi_gpios[PINCTRL_SW_OFF].gpioctrl != NULL)
			pinctrl_select_state(pinctrl_hifi, hifi_gpios[PINCTRL_SW_OFF].gpioctrl);

		if(hifi_gpios[PINCTRL_PDN_OFF].gpioctrl != NULL)
			pinctrl_select_state(pinctrl_hifi, hifi_gpios[PINCTRL_PDN_OFF].gpioctrl);
...
#else
	value = ak4376_reg_read(codec, 0x24);
#endif
...
#ifdef KERNEL_4_9_XX
    preLevel = dapm->bias_level;
#else
	preLevel = codec->dapm.bias_level;
#endif

	dev_dbg(codec->dev,"%s(%d) level=%d\n",__FUNCTION__,__LINE__,level1);
	dev_dbg(codec->dev,"%s(%d) pre bias_level=%d\n",__FUNCTION__,__LINE__, preLevel);

	switch (level1) {
	case SND_SOC_BIAS_ON:
		break;
	case SND_SOC_BIAS_PREPARE:
		if (preLevel == SND_SOC_BIAS_STANDBY) {
			dev_dbg(codec->dev,"%s(%d) pre bias_level == SND_SOC_BIAS_STANDBY\n",__FUNCTION__,__LINE__);
		}

		if (preLevel == SND_SOC_BIAS_ON) {
			dev_dbg(codec->dev,"%s(%d) pre bias_level >= SND_SOC_BIAS_ON\n",__FUNCTION__,__LINE__);
		}
		break;
	case SND_SOC_BIAS_STANDBY:
		if (preLevel == SND_SOC_BIAS_PREPARE) {
			dev_dbg(codec->dev,"%s(%d) pre bias_level == SND_SOC_BIAS_PREPARE\n",__FUNCTION__,__LINE__);
			//ak4376_pdn_control(codec, 0);
		} 
		if (preLevel == SND_SOC_BIAS_OFF) {
			dev_dbg(codec->dev,"%s(%d) pre bias_level == SND_SOC_BIAS_OFF\n",__FUNCTION__,__LINE__);
        }
		break;
	case SND_SOC_BIAS_OFF:
 /* 3.18
		if (preLevel == SND_SOC_BIAS_STANDBY) {
			printk("%s(%d) pre bias_level == SND_SOC_BIAS_STANDBY\n",__FUNCTION__,__LINE__);
			ak4376_pdn_control(codec, 0);
		}
 */
		break;
...
static int ak4376_set_dai_mute(struct snd_soc_dai *dai, int mute)
{
	...
	biasLevel = dapm->bias_level;
#else
	biasLevel = codec->dapm.bias_level;
#endif

	dev_dbg(codec->dev,"%s(%d) codec bias_level=%d\n",__FUNCTION__,__LINE__, biasLevel);
	...
	if ( biasLevel <= SND_SOC_BIAS_STANDBY) {
		dev_dbg(codec->dev,"%s(%d) codec bias_level <= SND_SOC_BIAS_STANDBY\n",__FUNCTION__,__LINE__);
...
static int ak4376_init_reg(struct snd_soc_codec *codec)
{
	...
	case 2:
		...
		chipID = ak4376_i2c_read(codec, AK4376_21_DUMMY);
		chipID &= 0x7;
		if ( chipID == 1 ) {
	...
	default:
		...
		//printk("This device are neither AK4375/A nor AK4376/A.\n");
	}
#ifdef AK4376_DETECT_IC
	if( ak4376->nDeviceID != 2 ){
		printk(KERN_INFO "Invalid Codc : Not ak4376/a Codec [%d]\n",ak4376->nDeviceID);
		ak4376->connected = 0;
		return 0;
	}
#endif
...
static const struct regmap_config ak4376_regmap = {
	...
	.max_register = (AK4376_MAX_REGISTERS - 1),    //  180509
                            ...
                            const struct i2c_device_id *id)
{
	...
	ak4376 = devm_kzalloc(&i2c->dev, sizeof(struct ak4376_priv), GFP_KERNEL);
	...
	ret = snd_soc_register_codec(&i2c->dev,
			&soc_codec_dev_ak4376, ak4376_dai, ARRAY_SIZE(ak4376_dai));
	if (ret < 0){
