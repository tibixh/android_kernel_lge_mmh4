...
#if defined(CONFIG_LGE_SPK_MUTE_CONTROL) && !defined(CONFIG_LGE_SPK_EXT_DAC_MUTE_CONTROL)
static int EXT_SPK_AMP_STATE = 0;
			    ...
			    struct snd_ctl_elem_value *ucontrol)
{
	...
	if (SPK_MUTE_FLAG == 1) {
		mCodec_data->mAudio_Ana_DevicePower[AUDIO_ANALOG_DEVICE_OUT_EXTSPKAMP] = 0;
		Ext_Speaker_Amp_Change(false);
	} else {
		if (EXT_SPK_AMP_STATE == 1) {
			Ext_Speaker_Amp_Change(true);
			mCodec_data->mAudio_Ana_DevicePower[AUDIO_ANALOG_DEVICE_OUT_EXTSPKAMP] = 1;
		}
	}
	return 0;
}
			       ...
			       struct snd_ctl_elem_value *ucontrol)
{
	...
	if(unlikely(SPK_MUTE_FLAG == 1)) {
		pr_info("%s(), skip change ext spk amp state\n", __func__);
	} else {
	...
	EXT_SPK_AMP_STATE = ucontrol->value.integer.value[0];
#else
	if (ucontrol->value.integer.value[0]) {
		Ext_Speaker_Amp_Change(true);
		mCodec_data->mAudio_Ana_DevicePower
...
static bool mExtAmpStereoSPK = false;
static bool mExtAmpMonoSPK = false;
static bool mExtAmpRCV = false;
static bool mExtAmpSPKHPH = false;
static bool mExtAmpHPH = false;
static bool mExtAmpLRSwap = false;

static int Ext_Amp_Stereo_SPK_Switch_Get(struct snd_kcontrol *kcontrol,
			  struct snd_ctl_elem_value *ucontrol)
{
	if(mExtAmpStereoSPK) {
		ucontrol->value.integer.value[0] = 1;
...
static int Ext_Amp_Stereo_SPK_Switch_Set(struct snd_kcontrol *kcontrol,
			  struct snd_ctl_elem_value *ucontrol)
{
	if (ucontrol->value.integer.value[0] == true) {
		mExtAmpStereoSPK = true;
#if defined(CONFIG_SND_SOC_AW87349) && defined(CONFIG_SND_SOC_AW87529)
		aw87349_audio_off();
		aw87529_audio_off();
		usleep_range(1 * 1000, 2 * 1000);
		...
		udelay(500);
#endif
		mExtAmpStereoSPK = false;
	}
	return 0;
}
static int Ext_Amp_Mono_SPK_Switch_Get(struct snd_kcontrol *kcontrol,
			  struct snd_ctl_elem_value *ucontrol)
{
	if(mExtAmpMonoSPK) {
		ucontrol->value.integer.value[0] = 1;
...
static int Ext_Amp_Mono_SPK_Switch_Set(struct snd_kcontrol *kcontrol,
			  struct snd_ctl_elem_value *ucontrol)
{
	if (ucontrol->value.integer.value[0] == true) {
		mExtAmpMonoSPK = true;
...
#if defined(CONFIG_LGE_SPK_MUTE_CONTROL) && !defined(CONFIG_LGE_SPK_EXT_DAC_MUTE_CONTROL)
static const struct soc_enum SpeakerMute_Enum = SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(amp_function), amp_function);
#endif
...
#endif
#if defined(CONFIG_LGE_SPK_MUTE_CONTROL) && !defined(CONFIG_LGE_SPK_EXT_DAC_MUTE_CONTROL)
	SOC_ENUM_EXT("Speaker_Mute_Switch", SpeakerMute_Enum,
		     Speaker_Mute_Switch_get, Speaker_Mute_Switch_set),
#endif
#ifdef CONFIG_LGE_STEREO_SPEAKER
	SOC_ENUM_EXT("Ext_Amp_Stereo_SPK_Switch", Stereo_Speaker_Enum[0],
		     Ext_Amp_Stereo_SPK_Switch_Get, Ext_Amp_Stereo_SPK_Switch_Set),
	SOC_ENUM_EXT("Ext_Amp_Mono_SPK_Switch", Stereo_Speaker_Enum[1],
		     Ext_Amp_Mono_SPK_Switch_Get, Ext_Amp_Mono_SPK_Switch_Set),
